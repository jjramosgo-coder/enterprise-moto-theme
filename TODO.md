...
| 32 | fix | **Tokenizar el color en el CSS del tema (retirar literales hardcodeados).** Origen: surgido al documentar la «Paleta Editorial» (§3.2 del manual de marca); un escaneo de solo lectura del CSS (`assets/css/*.css` + `style.css`) reveló hardcodeo de color extendido. La evidencia es un escaneo, **no** una auditoría exhaustiva: el Arquitecto la verifica en el código (localizar por patrón, no por línea). Cambio de **equivalencia de color** (mismo resultado visual), no rediseño. **Prerrequisito de contexto (obligatorio):** antes de desarrollarlo, el Arquitecto **lee el manual de marca** §3.2 «Paleta Editorial», §4.1 «Fondo y color» y las entradas de glosario «rol de color» y «Paleta Editorial» — se hace explícito porque las instrucciones de rol del Arquitecto **no** le obligan a leer el manual, y sin ese contexto no puede aplicar la frontera marca/funcional. **Frente (a) — deduplicar literales contra tokens existentes (mecánico):** sustituir por su `var(--…)` donde el literal es exactamente el valor del token — `#f2c118`(~18×)→`--gold`, `#0e0e0e`(~6×)→`--black`, `#1a1a1a`(~5×)→`--dark`, `#5a5a5a`→`--mid`, `#f5f5f2`→`--surface`, `#fff`/`#ffffff`→`--white`; los literales dentro de `rgba()` (p. ej. `rgba(14,14,14,.7)`=negro con alfa) no son sustituibles por `var()` directo y entran en el frente (b). **Frente (b) — definir el esquema de tokens para lo no tokenizado (decisión del Arquitecto), distinción rectora marca vs funcional:** candidatos de MARCA (alinear a la Paleta Editorial, §3.2) = escala de blanco translúcido `rgba(255,255,255,α)` α≈,025–,85 (texto atenuado sobre oscuro), escrim negro `rgba(0,0,0,α)`/`rgba(14,14,14,α)` α≈,25–,92, grises oscuros `#161616`/`#2a2a2a`/`#3a3a3a`, familia acento con alfa/variantes `rgba(242,193,24,α)`/`#d4b060`/`#BA7517`/`#8a5c1a`/`#4a3a1a`, grises/superficies claras `#f0f0f0`/`#e0e0e0`/`#f5f4ef`; candidatos FUNCIONALES (del tema, no de identidad; el Arquitecto define token y nomenclatura) = mapas azules `#2a3a5a`/`#080e1e`, estados/error rojos `#e03030`/`#E24B4A`/`#A32D2D`/`#ff6b6b`/`#ff0000`/`rgba(224,48,48,.15)`, estados/éxito verdes `rgba(26,154,74,.15)`/`#E1F5EE`, tip-box pastel `#FCEBEB`/`#FAEEDA`/`#E6F1FB`; regla de frontera: si un funcional debiera promoverse a marca, se consulta antes al Branding Copilot. **Tokens muertos** (0 referencias): `--light-mid #888` y `--surface-dk #f0f0ec` — decidir retirada (`limpieza`) o uso. **Condición de cierre (obligatoria):** antes de cerrar #32, el Arquitecto **redacta un TO-DO nuevo de tipo `branding`** para el Branding Copilot que reporte las decisiones del frente (b) —qué literales clasificó como MARCA (y a qué token de la Paleta Editorial los alineó o qué token de marca nuevo definió) y cuáles como FUNCIONAL—, para que el Branding Copilot incorpore al manual (§3.2) los colores que hayan pasado a ser de marca; el nuevo TO-DO toma el siguiente número global al crearlo. **Método/entrega:** trocear por temas (primero (a) mecánico, luego el esquema nuevo por familias), cada bloque testeable en WordPress; por el flujo Arquitecto → Desarrollador con su propio requirements y bump; convenciones del tema (`filemtime`, sin build). **Fuera de alcance:** cambiar valores de color o rediseñar la paleta. | pendiente |
| 33 | limpieza | **Retirar el bloque de CSS muerto `.cuaderno-h-*` de `style.css`.** Las reglas `.cuaderno-h-grid/-card/-thumb/-num/-body/-title/-excerpt` (`style.css` l.1018-1055, bajo el comentario `PLANTILLA CUADERNO HORIZONTAL`) estilizan un marcado que **ningún PHP del tema emite** — verificado: `cuaderno-h` solo aparece en `style.css`, tanto en el árbol actual como en toda la historia git visible; el commit raíz `97bf0b5` es una subida en bloque de la v2.4.6, así que el PHP original —si existió— quedó fuera del import. Código muerto al 100%: no renderiza nada. Retirar el bloque completo y quitar el selector `.cuaderno-h-card-link` de la regla compartida «quitar subrayado en cards» (l.1015), sin tocar el resto de esa regla. Inocuo. Surgió al preparar #32 (sus fallbacks incoherentes se van con el bloque). | pendiente |
| 34 | fix | **Cabecera de tabla del bloque markdown: acento dorado sobre superficie clara (falla de contraste).** `.ent-markdown th` (`markdown.css` l.92-94) pinta `color: var(--gold)` sobre `background: var(--surface)` → dorado `#f2c118` sobre crema `#f5f5f2`, que la §3.2 del manual de marca marca como falla (1,69-2,26:1). Verificado por especificidad (gana `.ent-markdown th` sobre el `th` global de `style.css:73`, que sí es legible: gris `--mid` sobre `--surface`) y porque `--surface`/`--gold` solo se definen en `:root`. Aplica al bloque **«Markdown»** (`enterprise/markdown`) y, por defecto, al **«Markdown con estilo»** (`enterprise/markdown-styled`). Causa: el token `--surface` pasó a claro y esta cabecera —diseñada oscura, fallback `#1a1a1a`— no se migró. **Decisión de diseño del arreglo (pendiente, con el manual delante):** restaurar dorado sobre fondo oscuro (`--dark`/`--black`, AAA) o alinear con la tabla global (gris sobre claro). Alcance: `markdown.css` l.81/93/103 (sus fallbacks oscuros vivos quedan aquí, **no** se barren en #32). Surgió al preparar #32; confirmado por Juanjo con el HTML renderizado. | pendiente |
| 35 | fix | **Galería de fotos / historias: superficie clara en un componente oscuro.** `.ent-gallery-stage` (`blocks-media.css:24`) usa `background: var(--surface)` = `#f5f5f2` (claro) y el `img` es `object-fit: contain` (l.62), así que el fondo asoma como **barras de letterbox claras** alrededor de las fotos, incoherente con el resto del componente (overlays `rgba(0,0,0,…)`, visor `#000`). Fallback `#1a1a1a` → diseñado oscuro. También `.ent-stories-thumb` (l.231) usa `var(--surface)` claro (visible solo en historias de icono; con imagen el `img` es `cover` y tapa el fondo). Misma causa que #34 (token `--surface` repurposado a claro, bloque no migrado) pero **otro bloque y otro síntoma** (barras vs. contraste de texto), por eso separado (invariante «no tomes el todo por la parte»). **Decisión del arreglo (pendiente, con el manual):** token oscuro correcto (`--dark`/`--black`). Alcance: `blocks-media.css` l.24/231 (sus fallbacks oscuros vivos quedan aquí, **no** se barren en #32). Surgió al preparar #32. | pendiente |
