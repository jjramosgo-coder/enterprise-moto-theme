...
| 13 | fix | **Persistencia y cobertura del contexto de regreso («Volver») más allá de un nivel de navegación.** Confirmado al implementar y validar #8: dentro de la colección la navegación funciona, pero al bajar de un viaje (tipo D) a una de sus etapas y **volver al viaje**, la cabecera pierde el contexto de colección y muestra el «Volver» genérico. **Traza verificada en código (líneas tras #8):** (a) `blocks/post-stages/render.php` estampa **solo** `from_post` en los enlaces de etapa (`$nav_suffix` l.37; enlaces l.114/152) y **no** propaga `from_col`; (b) el «Volver al viaje» de la rama `from_post` de `single.php` es `get_permalink( $from_post_id )` **plano** (l.37–39); (c) de vuelta en el viaje la URL ya no lleva `from_col` → fallback → «Volver» genérico. El contexto de origen es hoy de **un solo nivel**; se pierde en la navegación anidada viaje→etapa→viaje. **Cobertura desigual (mismo problema, más amplio):** la portada estampa `from_cat` solo si la sección tiene categoría resoluble (`functions.php` ~l.1826–1830); `archive.php` no estampa nada (enlaces planos, l.33/47/54). **Reutilizable de #8:** parámetros `from_col`/`col_key` y el helper `enterprise_collection_block_key()` (`functions.php` l.465; identidad estable de un bloque de filtrado por sus atributos de secuencia). **Decisiones a cerrar en conversación fresca (no diseñar aún):** (1) modelo de contexto — un origen que se **apila/propaga** a través del anidamiento vs. parámetros separados con reglas de coexistencia; (2) **prioridad** del «Volver» y de prev/next cuando coexisten `from_post` y `from_col`; (3) **qué se propaga hacia abajo** viaje→etapa: solo el nivel superior o toda la pila; (4) **alcance** — ¿incluye la cobertura uniforme en portada/`archive.php` o se trocea?; (5) confirmar que cada nivel sigue tomando el orden de **la misma fuente que genera su listado** (§13.1/§6). **Restricciones:** no romper `from_post`/`from_cuaderno`/`from_cat`/fallback ya validados; conservar «Ruta» fuera de colección y «Viaje» dentro (#8); `post-stages` es scaffolding compartido con `trip-collection` (no alterar su render base salvo lo necesario para propagar). Surgido al acotar #8; #8 §5 lo excluyó explícitamente. | pendiente |
