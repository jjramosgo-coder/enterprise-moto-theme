...
| 30 | branding | **Rediseño del footer: anclar la marca y compactar.** Sustituir el título de texto de la columna izquierda por el lockup (insignia BE + wordmark «BITÁCORA ENTERPRISE») y reducir el padding vertical. Columnas derechas (SECCIONES / BLOG) y barra inferior (copyright con el nombre del sitio) intactas. La marca visible de esa esquina pasa a «Bitácora Enterprise» (decisión de Juanjo). Apilado móvil conservado. Referencia visual: `claude/mockups/footer-rediseno-mockup.html`. Opcional si persiste hueco en monitores anchos: marca de agua tenue del monograma. Layout/CSS y responsive = Desarrollador; requiere requirements del Arquitecto. | pendiente |
