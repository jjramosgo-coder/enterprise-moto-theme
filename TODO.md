...
| 52 | limpieza | **[secuela de #51 · relacionado con #41] Motor de mapa content-agnostic + doc sin referencias al contenido del mapa.** Consolidar el principio «el mapa como artefacto es un asset; su **contenido** —qué países/regiones/islas aparecen— es contenido, no diseño». Tres tareas: **(1)** quitar `var OUTLIERS = { ES:['ESCN'] }` y su uso en `focusCountry` de `assets/js/region-map-frontend.js` (pasa a `childrenBBox` sin exclusión) → motor con **cero acoplamiento a contenido** (un mapa que no encuadre por sus islas es problema de contenido, no de código); requiere doc de requisitos, lo implementa el Desarrollador. **(2)** Reescribir **§13.19** del design doc: dejar solo mecanismo/arquitectura (SVG maestro único georreferenciado + árbol; motor DOM-only: navegabilidad/jerarquía/nombres del propio SVG, zoom por `viewBox`, vecinos visibles+clicables, regiones terminales foco-sobre-sí, globo bilingüe) y **eliminar toda referencia al contenido** (Canarias, lista de países, islas, recuentos por tier, `OUTLIERS`, las «decisiones E/F» que eran contenido). **(3)** Añadir a las **instrucciones del proyecto** (versionado) la regla: «el contenido del mapa se cambia con commit+push, **nunca bump**; el motor no lleva código ad-hoc por contenido». (2) y (3) son actos del Arquitecto tras validar (1). Surgido al retirar Canarias del maestro (jul 2026). | pendiente |
| 53 | doc | **[datos del mapa · doc] Documentar la asincronía ISO↔geometría vectorial (huérfanos) en design doc y manual de mapas.** Los nodos del árbol sin geometría en el maestro **no son un bug**: son el problema clásico de GIS de **asincronía entre estándares** —`pycountry`/ISO 3166-2 (al día) vs Natural Earth (geometría que va por detrás)—, que aflora cuando un país reorganiza divisiones (Cerdeña 2016, regiones de Francia 2016, etc.). Reflejarlo en dos sitios: **(a) design doc** — nota para futuros desarrollos, en especial las fases que **consumen el árbol** (#45/#46/#48), de que un desfase de códigos árbol↔geometría es **esperable y no bloquea** (el motor es content-agnostic e ignora el árbol; la coherencia se garantiza aguas arriba, en la generación), para que no entorpezca; **(b) manual de mapas** (`interactive-maps/manual-enterprise-maps.md`) — explicar al usuario el problema y **cómo mantener el diccionario de alias/*crosswalk*** (`NATURAL_EARTH_ALIASES`: Natural Earth → ISO vigente, aplicado **antes del `-join`** en Mapshaper) para cuadrar los códigos no sincronizados; advertir que el diccionario de partida es **solo-Europa y no exhaustivo** y cómo ampliarlo (identificar el ISO destino del código huérfano, p. ej. de los 7 actuales solo `ESNA→ESNC` y `ESPM→ESIB` están cubiertos). Actos del Arquitecto (documentación). Surgido del diagnóstico de la asincronía (jul 2026). | pendiente |
