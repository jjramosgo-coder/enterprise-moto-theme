...
| 45 | (sin clasificar) | **[#41 · Fase II.1 · Bloque II] Contrato de consumo de metadatos geográficos.** Definir/documentar cómo el tema **lee** las keys por nivel del metadato JSON de la etapa (Tipo B/C): qué keys, en qué niveles, con la **misma codificación** que las **propiedades de los objetos** del modelo de datos; nomenclatura por país y colapso; interfaz de lectura y comportamiento ante ausencia. **Fuera de alcance:** generación externa de las keys y **mecanismo de almacenamiento** (responsabilidad del Arquitecto ejecutor). Se aborda **tras** el Bloque I; solo depende del vocabulario de códigos de #42. Tipo a fijar al exportar/planificar. Requiere doc de requisitos. Ver §13.19. | pendiente |
...
| 54 | mejora | **Colores configurables del bloque «Mapa interactivo de regiones» (rehorneo de nivel 2).** El bloque `enterprise/interactive-region-map` expone un interruptor **«Colores del mapa»** con dos modos: **«Los del propio mapa»** (se ven los colores horneados en el activo SVG, nivel 1) y **«Personalizar»** (el tema los rehornea por CSS, nivel 2). En «Personalizar»: **desplegable de paleta** (`SelectControl`), **extensible** —hoy dos, «Editorial» y «Enterprise»—. **Paleta como juego cerrado:** la paleta seleccionada **limita** los colores usables exactamente a los suyos; cada atributo de color se elige con un swatch tipo `ColorPalette` alimentado con los swatches de la paleta activa (**patrón del bloque `markdown-styled`**), **sin color personalizado libre**. Atributos configurables = todos los permitidos por `map-style.schema.json` **salvo el canvas**: por `defaults` y por cada nivel `tier0`/`tier1`/`tier2`, los cuatro `fill`, `stroke`, `stroke-width`, `opacity`. Los **colores** (`fill`, `stroke`, acento hover) quedan **limitados a la paleta**; `stroke-width` y `opacity` son **numéricos** (sliders `RangeControl`) y **no** sujetos a esa restricción. Nombres de autor: *Relleno de las tierras* (`defaults.fill`), *Borde base* (`defaults.stroke`), *Borde de los países* (`tier0`), *Relleno/borde de las regiones* (`tier1`), *Relleno/borde de las provincias* (`tier2`). **Acento hover:** el realce al pasar el ratón deja de usar el literal dorado `#f2c118` actual de `path:hover`; pasa a un color propio **«Acento hover»** —aplicado por defecto incluso en modo «Los del propio mapa»— y en «Personalizar» elegido de la paleta. **Botón «volver»:** interruptor simple de **dos opciones**, «sobre lienzo claro» / «sobre lienzo oscuro» (sin pickers por parte del icono); replantea sus colores porque en el render real es **casi ilegible sobre el lienzo claro** (crema) actual. Se pinta sobre el **canvas de nivel 1**, **invariante no re-horneable** (§13.19): el autor **declara** el tipo de lienzo y el botón adapta su preajuste. **Invariante respetado:** el fondo del lienzo (`canvas`) **no es configurable** (§13.19); la coherencia de la combinación de colores es responsabilidad del autor. **Mecanismo:** atributos de bloque → clase en `.ent-region-map` → el CSS re-tiñe por `#tier0/1/2` y `[data-admin]` sobre los atributos de presentación horneados (escalón más bajo de la cascada; cualquier selector los sobrescribe); en modo «Los del propio mapa» no se emiten overrides. **Sería el primer bloque con `attributes`** (#43 lo registró deliberadamente sin ellos). **Requiere documento de requisitos** Arquitecto→Desarrollador (toca el registro PHP del bloque, el editor vanilla JS y el CSS). **Documentación al cerrar:** fija en **§13.19** la decisión del mecanismo de nivel 2 («tratarlo por CSS, con vuelta al nivel 1»); **cataloga el bloque en §7** (fila del catálogo) y **en §8** (subsección propia + precisión del encabezado sobre OpenLayers). **Sube versión** en los tres sitios al cerrar. **Relación con #47** (ref. cruzada; tema a analizar: orden de resolución región > paleta global > activo). Surgido en sesión de arquitectura jul 2026. | pendiente |
