...
| 48 | mejora | **[#41 · Fase III.3] Globo de hover completo.** Completar el globo con nombre (**nombre mostrado provisto por #49**, la capa de nombres localizada) + **nº de entradas** (consumiendo keys, #45) + descripción (#47) + pista de acción por nivel; estilado por tokens (§13.18); el conteo debe **casar con el listado del redirect** (#46) por usar la misma fuente. Cierra #41. Depende de #44, #45, #47, #49. Requiere doc de requisitos. Ver §13.19. | pendiente |
| 49 | mejora | **[#41] Capa de nombres de visualización localizada (es) para el mapa de regiones.** Los nombres de nivel-1 de `region-codes.json` salen en **inglés** (verbatim de `europe.svg`, p. ej. `Spain`/`France`), mientras los de región salen en local (`ESAN`→«Andalucía»). Definir una capa de nombres de visualización en español (al menos los 44 países de `level1`; armonización con los de región a decidir) que **#48 consumirá** (le provee el nombre mostrado del globo de hover). No re-genera los `id`; solo mapea código→nombre mostrado. Surgido al cerrar #42. Requiere doc de requisitos. Ver §13.19. | pendiente |
