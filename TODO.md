...
| 47 | mejora | **[#41 · Fase III.2] Presentación del mapa: realce por conteo, ancho de columna e higiene del SVG.** Fase de **pura visualización** del bloque `interactive-region-map`. El color de las regiones **ya viene dado** (horneado en el activo —nivel 1— y re-horneable por el tema —nivel 2, #54—): #47 **no** asigna color ni descripción por región y **no** gestiona las asociaciones etapa↔región de #45 (**frontera decidida: solo presentación**; la corrección de asociaciones vive en la capa de datos, #59). Contenido: **(1) Realce por `data-count`** (el atributo que inyecta #57) antes de renderizar — si `data-count="0"` **o el atributo está ausente** → reducir al **50 %** la opacidad de los colores de la región, **incluido el acento de hover**; si `data-count > 0` → color intacto y, en hover, **«levantar» ligeramente** la región (efecto de las cards), **sumándose** al hover actual (#54, `--ent-hover-accent`). **(2) [absorbe #62, `fix`]** el mapa **llena la columna de contenido**: retirar el tope `max-width: 900px` (y el `margin-inline:auto` inerte) de `.ent-region-map`, conservando `position:relative` (ancla «volver»); así el `wide`/`full` ya declarado es efectivo por instancia. Doc `claude/requirements/requirements-62-ancho-mapa-regiones.md` ya redactado. **(3) [absorbe #63, `limpieza`]** retirar el prólogo `<?xml version="1.0"?>` del SVG emitido (`render.php` hace `echo $svg`; en HTML5 es comentario bogus — marcado inválido). Mecanismo (CSS vs motor `region-map-frontend.js`) a fijar en el doc de requisitos. Depende de #43/#54/#57 (cerrados). **Absorbe #62 y #63** (ver sus filas en «Resueltas»). Requiere doc de requisitos. Ver §13.19. | pendiente |
| 48 | mejora | **[#41 · Fase III.3] Globo de hover completo.** Completar el globo con nombre (**nombre mostrado del modelo de datos del mapa** — `map-regions-global.json`, ya en español; ver el manual en `interactive-maps/`) + **nº de entradas** (consumiendo keys, #45) + descripción (#47) + pista de acción por nivel; estilado por tokens (§13.18); el conteo debe **casar con el listado del redirect** (#46) por usar la misma fuente. Cierra #41. Depende de #44, #45, #47. Requiere doc de requisitos. Ver §13.19. **Relación con #64:** este globo debe reaparecer en las regiones terminales tras el zoom (hoy no lo hace); #64 lo arregla — prerrequisito de la «pista de acción» y del globo extendido. **Entrada de #46 (ago 2026):** UX táctil del globo-enlace — en táctil la provincia es no-op al tocar la pieza y su globo-enlace solo aflora vía el reconcile del zoom → poco alcanzable; el globo completo debe resolver la afordancia táctil por nivel. | pendiente |
