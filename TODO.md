...
| 45 | mejora | **[#41 · Fase II · Bloque II] Capa de datos para el conteo por región y el filtrado de entradas del mapa.** Dar al tema una **capa geográfica consultable** que relacione las entradas con las unidades del mapa y alimente (a) el **conteo por región** de los globos y (b) el **filtrado de entradas** hacia el mapa. Unidad de asociación = **etapa (Tipo B/C)**, cuyo track GPX define las regiones que cruza. Modelo anclado (jul 2026): **taxonomía propia plana `regiones`** (términos = unidades del mapa) con **asignación de ancestros** (país+región+provincia) para que cada nivel se cuente directo; **clave de join** `region_code` (= `id` del `<path>` = código ISO); **siembra/actualización** idempotente desde el árbol JSON (`map-regions.schema`) vía **página de admin con botón** (previa, nunca borra, reporta divergencias); **asociación** vía bloque **«GPX metadata»** (un fichero de metadatos por entrada, en `uploads/routes/<ruta>/metada/`, leído por ruta local); **puerta de publicación** (solo si `validation_status: VALID` y todos los `administrative_regions[].id` existen como término); **asignación** con `wp_set_object_terms` (todos los niveles) → WordPress recalcula el `count` (solo publicados). **El globo cuenta etapas (Tipo B/C).** **Invariante duro:** posts soportados **solo Tipo B/C**. **Fronteras:** #45 = contrato de datos + conteo + **inyección de `data-count`**; **#48** = pintar el conteo en el globo; **#46** = redirect + agregación. **Naturaleza:** encargo de **diseño (Arquitecto→Arquitecto)**, no implementación → requiere el documento de requerimientos de diseño `claude/requirements/requirements-45-capa-datos-conteo-regiones.md`, cuya ejecución se descompondrá en **sub-TODOs** que reentran en el flujo Arquitecto→Desarrollador; el Arquitecto receptor puede reformular **#46/§13.19** (acotado: solo Tipo B/C, sin tocar la capa de datos). **Alimenta a #46 y #48.** Depende del vocabulario de códigos (#42) y del Bloque I (hecho). **Supera** el fraseo previo de §13.19 sobre «metadato JSON» como almacén (resuelto como taxonomía; a reflejar en §13.19 al cerrar). **[Encargo de diseño redactado jul 2026.]** **[Plan diseñado — 5 fases; jul 2026]** Descompuesto en **#58** (Fase 1 base: par mapa+árbol al almacén + render lee del almacén) → **#55** (Fase 1: taxonomía + «Sincronizar») + **#59** (Fase 1 mejora: migración de posts), **#56** (Fase 2: bloque «GPX metadata»+ingesta+puerta), **#57** (Fase 3: inyección de `data-count`); Fases 4 (#46/§13.19) y 5 (consolidación) sin sub-TODO propio (su salida vive en el documento-estado y §13.19). #45 es la raíz del plan. Ver §13.19. | planificado |
| 46 | mejora | **[#41 · Fase III.1] Redirect de nivel-3 a entradas filtradas.** El clic en la unidad de último nivel redirige a las entradas cuyas etapas casan sus keys (interfaz de #45), respetando el contrato de navegación (§6/§13.1) con un **nuevo contexto de origen `from_region`** integrado en `enterprise_nav_origin_params()` y el arrastre de ancestro (§13.11/§13.13); `routes-by-location` **como referencia, no reutilización**. **Punto de diseño mayor:** agregación etapa→viaje (Tipo D)→listados. Depende de #45 y del Bloque I. Requiere doc de requisitos. Ver §13.19. **Relación con #64:** el «globo extendido» que ofrece el salto vive en el globo de hover; #64 arregla que ese globo reaparezca en las terminales — resolver #64 antes de cablearlo. | pendiente |
