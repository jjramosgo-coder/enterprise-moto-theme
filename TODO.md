...
| 42 | mejora | **[#41 · Fase I.1] Activos del mapa de regiones en el tema.** Llevar a `assets/` versionado los SVG por país/nivel (es/it/fr admin1+admin2; pt distritos; ad parroquias) y **obtener el SVG de Europa** (nivel-1, un `<path id>` por país) del repositorio libre de simplemaps; **atribución** en `THIRD-PARTY-NOTICES.md`/`COPYRIGHT.md`; normalizar `viewBox`+`id`/`name`; **congelar la lista canónica de códigos** (alimenta el contrato #45). Variante ES (con islas / solo-península) a decidir en ejecución. Requiere doc de requisitos. Ver §13.19. | pendiente |
