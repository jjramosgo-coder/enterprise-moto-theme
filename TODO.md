...
| 14 | mejora | **Botón real «Última ruta» en el hero de la portada.** Hoy el rótulo «Última ruta» del hero (`<span class="hero-photo-tag">` sobre la imagen del hero, `index.php` ~l.103; estilado como pastilla dorada no interactiva en `style.css` l.267) **parece** un botón pero no lo es. Convertirlo en un enlace real que haga scroll hasta la sección destacada «Última ruta publicada» (`<section class="featured-section">`, `index.php` ~l.127), situada justo debajo del hero. Mecánica: (a) añadir un `id` de ancla a `.featured-section` (p. ej. `id="ultima-ruta"`); (b) convertir el `<span class="hero-photo-tag">` en `<a href="#ultima-ruta" class="hero-photo-tag">` — el scroll suave lo aporta el `html { scroll-behavior: smooth }` ya presente (`style.css` l.52), **sin JS**; (c) retoque CSS mínimo del tag para conservar apariencia como enlace (`text-decoration:none`, `cursor:pointer`, `display:inline-block` si hace falta); (d) accesibilidad: `<a>` real enfocable, valorar `aria-label` por texto corto. Ambos elementos se renderizan bajo el mismo `if ( $latest_post )`, así que el destino existe siempre que se muestre el botón (sin ancla colgante). Ficheros: `index.php` (ancla + markup del tag) y `style.css` (retoque del tag). **Decisión abierta (de Juanjo):** el texto del botón (hoy «Última ruta»; como afordancia podría ser «Ver la última ruta» o similar). | pendiente |
