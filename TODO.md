...
| 57 | mejora | **[#45 · Fase 3] Inyección de `data-count` por región en el render.** En `blocks/interactive-region-map/render.php`, inyectar `data-count` en cada `<path>` a partir del `count` de los términos (mapeo `path.id → region_code → count → data-count`), con **query agregada**, **estrategia de caché** (transient/object cache) e **invalidación**. Define la **frontera exacta con #48** (#45 inyecta el dato; #48 lo pinta en el globo). Conteo **basado en conjuntos** (un conteo por unidad). Resuelve §4.1. **Depende de #55 y #56; alimenta #48 y #46.** Requiere doc `requirements-57-*`. Ver §13.19. | pendiente |
