...
| 25 | otro | **Cabeceras de licencia/copyright en el código + coherencia de licencia GPL-3.0.** (a) Añadir bloque de copyright + `SPDX-License-Identifier: GPL-3.0-or-later` en `style.css` (dentro de la cabecera nativa del tema, sin sustituir sus campos) y en los PHP principales (`functions.php`, `single.php`, `footer.php`, `header.php`, `index.php`, y `render.php` de bloques). (b) Corregir el campo `License`/`License URI` de la cabecera de `style.css`: de «GNU General Public License v2 or later» / `gpl-2.0.html` → **v3 or later** / `gpl-3.0.html`. (c) `Theme Name` → «Bitácora Enterprise» (**la carpeta `enterprise-moto` y el text domain `enterprise-moto` NO se tocan**; solo el rótulo). Nombres de autores (Juanjo …, María José …) a confirmar al implementar. Requisito del proyecto de licenciamiento. Requerimientos: `claude/requirements/requirements-25-cabeceras-licencia-gpl3.md` (no versionado). | pendiente |
| 26 | otro | **Enlace del footer a la página «Licencia de contenido».** En `footer.php` (l. 54, dentro de `.footer-bottom`), el único enlace de la línea de copyright —el nombre del blog, que hoy apunta a `home_url('/')`— pasa a apuntar a la nueva página dedicada «Licencia de contenido» (reaprovechar el `<a>` existente, decisión de Juanjo; contra asumido: el nombre deja de enlazar a home). **Depende** de que la página exista y de conocer su slug/URL. Secuenciar después de crear la página. | pendiente |
