...
| 48 | mejora | **[#41 · Fase III.3] Globo de hover completo.** Completar el globo con nombre (**nombre mostrado del modelo de datos del mapa**, `map-regions.json`, ya en español — ver `manual-enterprise-maps.md`) + **nº de entradas** (consumiendo keys, #45) + descripción (#47) + pista de acción por nivel; estilado por tokens (§13.18); el conteo debe **casar con el listado del redirect** (#46) por usar la misma fuente. Cierra #41. Depende de #44, #45, #47. Requiere doc de requisitos. Ver §13.19. | pendiente |
...
| 51 | (sin clasificar) | **(BORRADOR — especificación de #51 en curso, fuera del flujo estándar; no accionar como definitiva)** **[#41 · deriva de #44] Zoom continuo del mapa de regiones conservando el contexto (modelo tipo simplemaps).** Evolución del motor de #44: sustituir el *swap* de SVG por país (que hace desaparecer los vecinos al entrar en un país) por una **navegación continua sobre un único mapa georreferenciado**, replicando la **técnica** del mapa JS+SVG de simplemaps.com/world (su librería es de pago; se copia la técnica con motor propio, coherente con #41/§13.19). **Navegación objetivo (especificación, validada como «lo que quiero» por el operador):** (1) **Un único mapa:** todos los países/regiones/provincias en **un solo SVG y un solo sistema de coordenadas** (una proyección); nunca se cambia de fichero. (2) **Zoom = animar el `viewBox`** hasta enmarcar el *bounding box* del área clicada (con margen), como un «vuelo» suave; misma técnica `getBBox`+`viewBox` que #44 ya usa dentro de un país. (3) **Los vecinos no desaparecen:** al hacer zoom, las áreas adyacentes siguen dibujadas pero **atenuadas** (opacidad reducida, tipo `adjacent_opacity`); ej.: al entrar en España se ven sus CC.AA. y Francia/Portugal/Andorra siguen visibles alrededor; al entrar en una región, sus provincias, sin que desaparezcan las de las vecinas. (4) **Volver = icono propio arriba-izquierda** (decisión de #44, sin texto) que **aleja un nivel** sobre el mismo lienzo. (5) **Móvil = tocar** + zoom suave (decisión de #44: sin pinch/pan). **Aproximación técnica (viabilidad probada jul 2026):** **SVG maestro georreferenciado** (países objetivo y sus niveles como capas en una sola proyección), generado con `mapshaper` desde geometría de Natural Earth (dominio público); códigos según autoridad **ISO 3166-2**; **GADM y Eurostat/NUTS descartados** (licencia y codificación/cobertura). Probado con Gemini/Colab: nivel-1 (ES/FR/PT/IT/AD en lienzo compartido, Andorra incluida) y provincias de España en el **mismo EPSG:3035** conservando el footprint y los vecinos; ids ISO (alpha-2 país / 3166-2 región·provincia) conforme al **modelo de datos del mapa (`map-regions.json` + `map-levels.json`, documentados en `manual-enterprise-maps.md`), que sustituye a `region-codes.json`** (obsoleto al completar #51). El motor `region-map-frontend.js` de #44 se adapta a **capas + `viewBox` + atenuación**, en vez de *swap*. **Pendiente / abierto:** derivar el nivel intermedio de **CC.AA.** (fusionar provincias→CC.AA. con `-dissolve`, o NUTS-2); tratamiento de **islas** (Canarias/Azores: posición real vs recuadro) y de **países-contexto** vecinos; **trabajo previo del operador** antes de formalizar. Deriva de #41 (raíz) y #44. **En evaluación / requiere doc de requisitos.** Ver §13.19. **[#51 · jul 2026] Modelo de datos del mapa (artefactos):** `manual-enterprise-maps.md` (raíz del proyecto, versionado) + `map-levels.json`, `map-regions.json`, `map-style.json` (en `claude/res/`). | pendiente |
