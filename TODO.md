...
| 51 | (sin clasificar) | **[#41 · deriva de #44] Zoom continuo del mapa conservando el contexto entre países.** El motor de #44 hace *swap* de SVG por país, de modo que al entrar en España desaparecen Francia/Portugal/Andorra. Se desea un zoom continuo que mantenga visibles los vecinos (países y regiones/provincias contiguas) al acercarse. **Medido (jul 2026):** no se logra fusionando los activos actuales — `europe.svg` y los SVG de país no comparten proyección ni coordenadas (aspecto de la misma península 1,31 en `europe` vs 1,24 en `es-regiones`); sí comparten coordenadas los niveles **dentro** de un país (bbox `es-regiones`≡`es-provincias`). Requiere **activo georreferenciado único** (todos los países objetivo y sus niveles en una sola proyección, desde datos GIS abiertos) o motor geográfico (d3-geo/GeoJSON). **En evaluación de opciones.** Deriva de #41 (raíz) y #44. Requiere doc de requisitos. Ver §13.19. | pendiente |
