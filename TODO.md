...
| 17 | mejora | **Bloque nuevo «Mapa de rutas por localización»** (`enterprise/routes-by-location`; mejora sustancial; toma `location-map` solo como referencia — el actual **no se toca**). Cada marcador (localización) guarda nombre, coordenadas, descripción **opcional** y un **filtro compuesto** sobre las taxonomías existentes: **(cat_1 OR … OR cat_n) AND (tag_1 OR … OR tag_m)** (vocabulario `_filt_*`/§7, reutilizando `enterprise_stage_query()`, `functions.php:388`). El popup —misma estructura que el actual— lleva un enlace **«→ Entradas relacionadas»** a la página-destino que auto-compone el grid de las entradas que casan el filtro (incluye etapa/jornada/viaje sin re-etiquetar). Alta con el **buscador Nominatim** ya existente (nombre → coordenadas) **más clic en el mapa**; gestión de decenas/cientos mediante **Modal** (contenedor) con **mapa OpenLayers + DataViews/DataForm** (WP 7.0), sustituyendo la lista de la barra lateral que no escala. Almacenamiento **por-bloque** (atributos); reutilización = copiar el bloque (copy/paste conserva marcadores). En esta iteración el Desarrollador implementa además una **página-destino provisional sencilla** (lee el filtro de parámetros de URL y pinta el grid vía `enterprise_stage_query()`) para validar el bloque; su rediseño definitivo es **#18**. Bump al cierre **2.7.3 → 2.8.0**. **Fuera de alcance:** contexto de navegación prev/next del destino (§6). Requisitos: `claude/requirements/requirements-17-routes-by-location.md` (por redactar). | pendiente |
| 18 | mejora | **Rediseño definitivo de la página-destino de «Mapa de rutas por localización» (#17).** El bloque #17 enlaza cada localización a una página que muestra el grid de entradas que casan su filtro compuesto **(cat OR…) AND (tag OR…)**. En #17 el Desarrollador implementa una **página provisional sencilla** solo para validar el bloque. Este TO-DO (**para una IA Arquitecto**) es el **rediseño definitivo** de esa página, que **sustituye** a la provisional: diseño/estética según mockups (`claude/mockups/`), decisión de URL/enrutado limpio (Página con plantilla específica / rewrite / etc.; **confirmar antes qué es `/las-rutas/`** — hoy solo aparece como `home_url('/las-rutas/')` en enlaces, sin manejador en código), y el **contexto de navegación prev/next** del destino (§6/§13.1; un `from_loc`/`from_tag` sería un tipo de contexto nuevo). Depende de #17. | pendiente |
