...
| 22 | fix | **La relación entre las etiquetas de una localización del bloque «Mapa de rutas por localización» debe ser AND, no OR.** La especificación original de #17/#18 fijó el filtro compuesto como `(cat_1 OR … OR cat_n) AND (tag_1 OR … OR tag_m)` (etiquetas con OR) por error de diseño del arquitecto; lo correcto es AND entre etiquetas: una entrada casa solo si lleva **todas** las etiquetas del marcador → `(cat OR …) AND (tag AND …)`. La relación no se guarda por marcador ni viaja en la URL; se fija al resolver, en los **dos** únicos puntos (`page-templates/template-routes-by-location.php` l.90 y `single.php` l.526), que deben cambiar juntos (contrato de navegación §6). `enterprise_stage_query()` **ya soporta** AND (`operator => 'AND'`) — no se toca (su default IN lo usan `post-stages`/`trip-collection`). Sin cambio de editor, atributos ni datos. Requerimientos: `claude/requirements/requirements-22-fix-tags-and-routes-by-location.md` (no versionado). | pendiente |
