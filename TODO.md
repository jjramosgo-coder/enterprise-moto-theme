...
| 20 | fix | **«Mostrar numeración» debe gobernar también el pin, no solo la leyenda.** En `location-map` y `routes-by-location` el interruptor `showNumbers` solo afecta a la leyenda; el pin va siempre numerado porque el flag no se emite al frontend (`.ent-map` no lleva `data-show-numbers`) y `olPinStyle` se llama siempre con número. Fix: emitir `data-show-numbers`; con el toggle desactivado, el pin muestra un **punto centrado** en lugar del número (`olPinStyle(null)`). Cubre **ambos** bloques (decisión de Juanjo). Requisitos en `claude/requirements/requirements-20-fix-numeracion-pines-mapas.md`. Surgido al revisar #17. | pendiente |
| 21 | fix | **«← Volver al mapa» del destino RBL se pierde tras navegar por las entradas filtradas.** Al volver desde una entrada, la URL del destino no lleva `rbl_src`, así que el botón cae al `wp_get_referer()` (la propia entrada recién dejada) en vez de al mapa. Causa: `rbl_src` no se propaga en la frontera destino→tarjeta→entrada. Fix: propagar el id del mapa como `loc_src` por toda la cadena (tarjetas, prev/next, ancestro) y reponerlo como `rbl_src` al pulsar «← Volver». Requisitos en `claude/requirements/requirements-21-fix-volver-al-mapa-rbl-src.md`. Surgido al revisar #18. | pendiente |
