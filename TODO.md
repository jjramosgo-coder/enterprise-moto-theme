...
| 13 | fix | **Persistencia y cobertura del contexto de regreso («Volver») más allá de un nivel de navegación.** Síntoma (observado en WordPress + verificado en código): el «Volver» de una entrada regresa al origen correcto solo parcialmente. **(a) Cobertura inconsistente:** la portada estampa `?from_cat` únicamente cuando la sección tiene categoría resoluble (`functions.php` ~l.1826–1830) — categorías como «de vacaciones» caen al fallback del referer, mientras «de puente» sí vuelve al archivo de categoría en grid (comportamiento que gusta y se conserva); `archive.php` no estampa `from_cat` (enlaces planos, l.33/47/54). **(b) Pérdida por anidamiento (raíz del problema):** al bajar de un viaje (tipo D) a una de sus etapas, `post-stages` estampa **solo** `from_post` en los enlaces (`render.php` l.37/114/152) y no propaga el contexto externo; al «Volver al viaje» (`single.php` l.29–32, permalink plano) el viaje ya **no** conserva su `from_cat` original y su «Volver» cae al fallback. El contexto es hoy de **un solo nivel** y de cobertura desigual. **Dirección (a decidir al abordarlo):** estampar contexto de forma uniforme en los orígenes que enlazan a entradas (portada, `archive.php`…) y **propagar/apilar** el contexto a través de la navegación anidada, para que «Volver» regrese siempre a la fuente semántica real, respetando el contrato §13.1/§6. Más profundo que #8 (que solo añade el origen «colección» vía `from_col`); surgido al acotar #8. | pendiente |
