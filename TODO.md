...
| 56 | mejora | **[#45 · Fase 2] Bloque «GPX metadata» + ingesta y puerta de publicación.** Bloque que **referencia un fichero de metadatos por entrada Tipo B/C** (`uploads/routes/<ruta>/metada/`, leído por **ruta local**) como portador de la asociación. **Ingesta** al guardar/publicar: valida (`validation_status == "VALID"` **y** todos los `administrative_regions[].id` existen como término) → `wp_set_object_terms` (**todos los niveles**: país+región+provincia) → WordPress recalcula el `count` (solo publicados). **Puerta de publicación** que bloquea si no se cumple. Resuelve §4.3 (diseño del bloque, comportamiento si hay más de uno), §4.6 (lectura por ruta local) y §4.4 (mecanismo de la puerta). Invariante: **solo Tipo B/C**. **Depende de #55.** Requiere doc `requirements-56-*`. Ver §13.19. | pendiente |
