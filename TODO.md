...
| 55 | mejora | **[#45 · Fase 1] Capa de datos: taxonomía `regiones` + siembra idempotente (herramienta «Sincronizar»).** Alta de la taxonomía propia **plana** `regiones` (etiqueta «Regiones») sobre `post`, **registrada mínima en la parte pública** (sin páginas/URLs por región; Decisión D — el destino del clic es de #46), con term meta `region_code` (= `id` del `<path>` = código ISO, clave de join) y `region_admin` (nivel). **Herramienta «Sincronizar regiones»** en la página del menú propio **«Enterprise Moto»** (Decisión A): siembra/actualización **idempotente** desde el **árbol autoritativo** (en el almacén del sitio vía #58; copia del tema como respaldo), con **vista-previa-y-confirmar siempre** (Decisión C), **empareja por código** (Decisión E) y **reporte de divergencias no destructivo** —nunca borra términos ni reasigna posts; «actualizar» = solo el nombre; término con posts protegido; separa descolgado con/sin posts (Decisión F)—. Precedente reutilizable: `enterprise_ensure_cuaderno_terms` (`functions.php` l. 3196–3207). Cambiar las unidades del mapa es op de contenido (sin bump). **Depende de #58** (par en el almacén) y del vocabulario de códigos (#42, hecho). La **migración** de posts es #59. Requiere doc `requirements-55-*`. Ver §13.19. | pendiente |
