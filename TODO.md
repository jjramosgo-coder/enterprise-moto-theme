...
|---|------|-------------|--------|
| 3 | fix | Unidad «km» defensiva en las tarjetas de entradas filtradas del **cuaderno** (`page-cuaderno-de-bitacora.php`, vistas carrusel `.ps-card-km` y timeline `.exp-tl-km`) — mismo defecto que #2 en «Etapas de ruta». Se resuelve con un **helper de presentación compartido** `enterprise_km_display()` en `functions.php`, usado también en los dos puntos de post-stages (sustituyendo el inline de #2). Análisis conservado más abajo. | pendiente |
