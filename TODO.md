...
| 43 | mejora | **[#41 · Fase I.2] Esqueleto del bloque `enterprise/interactive-region-map` + render estático.** Registrar el bloque (render PHP, convención §7); render **inline** del SVG de nivel-1 (Europa) en el front (inline porque la interactividad de #44 lo necesita); encolado solo si el bloque está presente + `filemtime()` (§7/§13.9). Coordinación con **#39** (categoría de bloques) a decidir en ejecución. Depende de #42. Requiere doc de requisitos. Ver §13.19. | pendiente |
