...
| 48 | mejora | **[#41 · Fase III.3] Globo de hover completo.** Dar al globo del mapa su forma final. Cuatro piezas cerradas + una pendiente de arbitrar: **(1) Nombre** de la región en español — *ya entregado* (el motor pinta `data-name`, nombre bilingüe separado con `parseName().primary`). **(2) Nº de entradas:** etiqueta «**Rutas: N**» delante del número, en el estado con conteo (lee `data-count`, #57). **(3) Pista de acción:** el globo invita a ver el contenido **solo** en las piezas terminales con `data-count>0`, con texto tipo «**Ver las rutas**»; se **elimina** la pista «acercar» (el gesto es evidente) y se **descarta** la «descripción» por región (anulada al redefinir #47). **(4) Estilo por tokens (§13.18),** sin colores sueltos; prioridad: combinación **legible sea cual sea el color del mapa** (lo fija el autor con #54) — verificar contraste antes de fijarla. **(5) [pendiente de arbitrar]** Que el globo funcione con el **dedo** (móvil/tablet), no solo con ratón, y **dónde queda anclado** al tocar/clicar el último nivel de una entrada con rutas: (a) cómo responde el toque por nivel para que el enlace-globo sea igual de alcanzable que con ratón; (b) posición del globo (punto tocado / centro de la pieza / fija). *Invariante (no es tarea):* el conteo del globo casa por construcción con el listado de #46 (misma fuente, #57). Cierra #41. Depende de #44, #45, #47 y de **#64** (prerrequisito, ya cerrado v2.18.1). Requiere doc de requisitos. Ver §13.18/§13.19. | pendiente |
