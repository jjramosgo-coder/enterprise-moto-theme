...
| 62 | fix | **El mapa interactivo de regiones no llena la columna de contenido.** `.ent-region-map { max-width: 900px }` (`interactive-region-map.css` l. 7-11) topa el mapa a 900px < ~1160px de columna útil (`--max:1240px` − padding, `style.css` l. 79/711), dejando aire a los lados en pantallas anchas; ese tope además **neutraliza** el `align` wide/full que el bloque **ya declara** (`functions.php` l. 1579; el tema tiene `add_theme_support('align-wide')`, l. 53). Decisión (operador, 2026-08-02): **llenar la columna** — relajar el tope para que a alineación por defecto el mapa llene la columna y el wide/full (ya soportado, estilado en `.entry-content`, `blocks.css` l. 7-8) sea efectivo por instancia; preservar `position:relative` (ancla «volver»). Relación 800×533 ≈ 1.5:1 (más ancho, más alto). Requisitos: `claude/requirements/requirements-62-ancho-mapa-regiones.md` (redactado, 1 commit + bump 2.18.0 → 2.18.1). Corrige el diagnóstico previo que daba el align por no declarado. Ver §8. | pendiente |
| 63 | (sin clasificar) | **[limpieza propuesta] El activo `enterprise-eu.svg` empieza por el prólogo `<?xml version="1.0"?>` y `render.php` lo emite tal cual** dentro de `.ent-region-map` (`echo $svg`). En HTML5 ese prólogo se parsea como **comentario bogus** (no es texto visible, matiz que corrige el «se cuela como texto»), pero es marcado inválido: retirarlo del SVG emitido (en `render.php` antes del `echo`, o del propio activo). Verificar y acotar al redactar el requisito. **Separado de #62** (observación del Desarrollador, 2026-08-02). | pendiente |
