...
| 27 | branding | **Identidad de marca en el tema: favicon y logo del header.** Incorporar el set de favicon de «Bitácora Enterprise» producido por Branding en `claude/bocetos/favicon/` (`favicon.svg`, `favicon.ico` 16/32/48, `apple-touch-icon.png` 180, `android-chrome-192x192/512x512.png`, `favicon-16/32/48.png`; «BE» simplificado para tamaños pequeños y mark completo con aguja para los grandes; letras trazadas desde Bebas Neue, sin dependencia de fuente). Crear `enterprise-moto/assets/images/` como hogar de imágenes propias (© reservado, `COPYRIGHT.md`) y colocar el set. **Decisión pendiente de Juanjo/Arquitecto:** fuente única del favicon — Site Icon de WordPress vs emisión propia en `wp_head` desde el tema (recomendación de Branding: en el tema; admite SVG y `.ico`, versionado y consistente). Si se opta por el tema, desacoplar el logo del header (`header.php` usa hoy `has_site_icon()`/`get_site_icon_url(40)` y sin Site Icon cae al emoji 🏍️). Ejecutar junto a #28. Requiere requirements del Arquitecto. | pendiente |
| 28 | limpieza | **Carpeta espuria `enterprise-moto/assets/{css,js,images}`.** Carpeta vacía cuyo nombre es literalmente `{css,js,images}` (resto de un `mkdir` sin expansión de llaves); `css/` y `js/` reales existen, pero `images/` no. Borrarla. Inocua. Detectada al preparar #27; se ejecuta en la misma sesión que #27, donde `assets/images/` se crea. | pendiente |
