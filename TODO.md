...
| 64 | fix | **El globo de nombre no reaparece en las 33 regiones terminales (un clic) tras el zoom.** En `assets/js/region-map-frontend.js` el globo **solo se enciende** en el manejador `mouseover` (`showBalloon` al **entrar** en una pieza) y se apaga al salir (`mouseout`) y al inicio de cada zoom (`animateVB`→`hideBalloon`). `moveBalloon` (l. 208) **no enciende**: si el globo está oculto retorna sin hacer nada, así que **mover el ratón no puede devolverlo**. En una región terminal el clic hace `setActive` sobre **la misma pieza** y zoom sobre ella (`focusRegion`, rama terminal); al terminar, el cursor sigue **dentro de la misma pieza**, no cruza a ninguna nueva y `mouseover` no se re-dispara → el globo no vuelve (desktop). En táctil el navegador finge `mouseover` solo al tocar una pieza **distinta** de la última, por lo que re-tocar la misma terminal tampoco lo enciende (móvil/tableta). Las **41** regiones de tres niveles **no** lo sufren porque al pinchar se **oculta** la región y se **revelan** provincias (pieza nueva bajo el cursor → `mouseover`). **Solución propuesta (dos frentes):** (1) **ratón** — que `mousemove` pueda **encender** (mirar la pieza bajo el cursor: apuntable → mostrar globo con su nombre; si no → ocultar), no solo reposicionar; (2) **táctil** — mostrar el globo **explícitamente al enfocar una terminal por toque**, posicionado en el punto tocado, al acabar el zoom. Bug del **motor**, **anterior a #45** e independiente de la capa de datos; diagnóstico verificado en código (no ejecutado — validación en el WordPress real de Juanjo). **Relación con #48/#46:** el «globo extendido» que ofrecerá el salto a posts vive en este mismo globo → conviene resolver **#64 antes** de cablear esa afordancia. Requiere doc de requisitos para el Desarrollador. Surgido en sesión de arquitectura de #45 (2026-08-02). | pendiente |
