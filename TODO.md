...
| 34 | fix | **Cabecera de tabla del bloque «Markdown» (`enterprise/markdown`): texto dorado sobre banda casi blanca, ilegible.** `.ent-markdown th` (`markdown.css` l.92-99) pinta `color: var(--gold)` (dorado `#f2c118`) y un subrayado `border-bottom` dorado (l.99) sobre `background: var(--surface)` (crema `#f5f5f2`). **Diagnóstico corregido:** el contenido del artículo es de **fondo blanco** (`body { background: var(--white) }`; `.entry-content` = texto oscuro sobre claro) — lo oscuro del tema es solo el *chrome* (nav/footer/hero/portal). Es, por tanto, **dorado sobre casi-blanco** → contraste ~1,9:1, que la §3.2 del manual prohíbe como texto; en cabecera de tabla es texto pequeño (`.9em`) e ilegible. El fallback `#1a1a1a` era ruido: la cabecera nunca fue oscura. Aplica a «Markdown» y, por defecto, a «Markdown con estilo» (`enterprise/markdown-styled`). **Solución (confirmada por Juanjo):** alinear al estilo de tabla vigente del tema — el `th` global (`style.css:72`) es `color: var(--mid)` (gris `#5a5a5a`) sobre `background: var(--surface)` (crema), legible sobre blanco (AA, 6,3:1): **misma banda crema, texto gris en vez de dorado**, y retirar el trazo dorado del `border-bottom` (dorado sobre claro, mismo motivo). Alcance: `markdown.css` `.ent-markdown th` (l.92-99). Los encabezados `h1-h4` del bloque, también dorados sobre blanco, son tipografía grande decorativa y quedan **fuera** de #34. Surgió al preparar #32. | pendiente |
| 35 | (sin clasificar) | **(Por confirmar visualmente) Fondo de stage del «Carrusel de fotos» (`enterprise/photo-gallery`) y del thumb de «Stories» (`enterprise/stories`).** `.ent-gallery-stage` (`blocks-media.css:24`) y `.ent-stories-thumb` (l.231) usan `background: var(--surface)` (crema `#f5f5f2`). **Diagnóstico corregido:** ambos se renderizan en el contenido del post, de **fondo blanco** — NO en un componente oscuro (el `#000` es solo el *lightbox* `.ent-lb-*` del «Carrusel de fotos» a pantalla completa, otro estado del bloque). Como el `img` del carrusel embebido es `object-fit: contain` (l.62), el fondo asoma como letterbox, pero **crema sobre blanco es casi invisible** → probablemente **no es un bug**, o es un matiz mínimo. El marco original («componente oscuro / barras claras») era erróneo (fondo invertido). **Pendiente:** que Juanjo confirme qué se ve en un «Carrusel de fotos» embebido; entonces se reclasifica (`fix` si molesta, o se cierra como no-bug). Surgió al preparar #32. | pendiente |
