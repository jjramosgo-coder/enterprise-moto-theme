...
| 15 | fix | **`location-map`: el enlace del popup no navega + comentario de cabecera desfasado (mismo componente, un solo TO-DO).** **Síntoma** (reportado por Juanjo; verificar sobre HTML renderizado, no de memoria): al añadir un marcador con `postUrl` (caso real `https://bitacoraenterprise.com/pirenaica-oeste-2026/`), el popup pinta el rótulo «→ Leer la entrada» pero el **clic no navega**. **Verificado en código:** el `<a>` **sí se emite** correctamente — `popupHtml()` en `assets/js/map-frontend.js` (≈L181-186) construye `'<a class="ent-map-popup__link" href="'+url+'">→ Leer la entrada</a>'` con la URL del marcador (`_url`), y el popup es un `ol.Overlay` (`makePopup()`, ≈L169). Por tanto el fallo **no** está en generar el enlace sino en la **capa de interacción** — *hipótesis a confirmar, no dar por hecha*: el clic no llega al ancla (posible captura de eventos en fase capture del overlay/mapa —protección de scroll §7— o `pointer-events`/CSS del popup). Diagnóstico **por comparación** sobre código real + HTML/consola del navegador; distinguir hecho de hipótesis. **En el mismo paso**, corregir el comentario de cabecera de `blocks/location-map/render.php` («La lógica Leaflet está en assets/js/map-frontend.js») → el motor real es **OpenLayers 9.2.4** (§8); solo comentario, no cambia comportamiento. **Para el Arquitecto que lo tome:** redactar el documento de requerimientos en `claude/requirements/requirements-15-fix-enlace-popup-location-map.md` siguiendo `claude/templates/requirements-authoring-guide.md`; el programador implementa y entrega fichero completo + comandos git (por fichero, agrupado por tema); cierre por el arquitecto (doc §8 si procede; bump a decidir al cerrar el lote). | pendiente |
