...
| 16 | limpieza | **Purga completa de Leaflet** (motor real de mapas: OpenLayers 9.2.4; Leaflet **confirmado sin uso** — no se encola en ningún punto, sin API `L.` en el JS). Alcance ampliado (jul 2026): (a) borrar `enterprise-moto/assets/vendor/leaflet/` (`leaflet.js` + `leaflet.css`, nunca encolados); (b) retirar el CSS muerto `.leaflet-*` de `assets/css/maps.css` (bloque «Override estilos Leaflet», ≈l. 70-88) y reescribir el comentario de l. 29; (c) corregir el comentario de cabecera desfasado «LEAFLET EN FRONTEND» de `functions.php` (la función encola OpenLayers, no Leaflet); (d) retirar la entrada de Leaflet de `THIRD-PARTY-NOTICES.md`. Inocuo, no cambia comportamiento. Surgido al cerrar #15. Requerimientos: `claude/requirements/requirements-16-purga-leaflet.md` (no versionado). | pendiente |
