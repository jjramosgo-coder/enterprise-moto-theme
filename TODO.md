...
| 41 | mejora | **Nuevo bloque «Mapa interactivo de regiones».** Diseñar, implementar e integrar un bloque Gutenberg que convierta un SVG de regiones administrativas en un mapa interactivo (zoom, clic, optimizado para móvil) al estilo del personalizador de simplemaps, con **motor propio/libre** (su librería interactiva es de pago; sus SVG estáticos son libres, atribución agradecida). Personalización por región (color, descripción, URL…) según la guía editorial (tokens de color, §13.18). Interacción: *hover* con globo (contenido a definir) y clic que hace zoom entre **tres niveles** administrativos (nomenclatura a fijar); en el último nivel, el clic **redirige a las entradas filtradas por el área**, tomando el bloque/plantilla `routes-by-location` y su filtrado **como caso de referencia** y **repensando el filtrado del nuevo bloque en sus propios términos**. **Paradigma alternativo** (regiones SVG) que **complementa —nunca sustituye—** la funcionalidad de los bloques OSM/OpenLayers (tiles, lat/lng): herramientas distintas para trabajos distintos; no reutiliza su motor. Los mapas SVG generados se alojan en `claude/res/svg-maps/`. Por tamaño y por decisiones abiertas, **no va directo a Desarrollador: requiere primero un documento de requisitos que encargue a un Arquitecto el diseño y la documentación de un plan de proyecto** (fases, decisiones de arquitectura, sub-TODOs); brief en `claude/requirements/closed/requirements-41-mapa-regiones-interactivo.md` (planificación, archivado tras diseñar el plan). Surgido en sesión de arquitectura jul 2026. **[Plan diseñado jul 2026]** Decisiones de arquitectura en **§13.19** del design doc; ejecución descompuesta en las fases **#42–#48**; **#41 es el TO-DO raíz del plan**; su ejecución se encarga mediante el documento de requerimientos (encargo Arquitecto→Arquitecto) `claude/requirements/requirements-41-ejecucion-mapa-regiones-interactivo.md` y se descompone en #42–#48. **[jul 2026]** Derivados nuevos tras cerrar #44: **#50** (accesibilidad de teclado/lector retirada de #44) y **#51** (rehízo la Fase I con SVG único georreferenciado; **cerrada en v2.16.0**, ver Resueltas y §13.19). | planificado |
| 48 | mejora | **[#41 · Fase III.3] Globo de hover completo.** Dar al globo del mapa su forma final: **(1) Nombre** de la región en español — *ya entregado* (el motor pinta `data-name`, nombre bilingüe separado con `parseName().primary`). **(2) Nº de entradas:** etiqueta «**Rutas: N**» delante del número, en el estado con conteo (lee `data-count`, #57). **(3) Estilo por tokens (§13.18),** sin colores sueltos; prioridad: combinación **legible sea cual sea el color del mapa** (lo fija el autor con #54) — verificar contraste antes de fijarla. La **pista de acción** del globo y su **uso táctil por nivel + anclaje** se **extraen a #66** (mejora menor de UX, no bloqueante). *Invariante (no es tarea):* el conteo del globo casa por construcción con el listado de #46 (misma fuente, #57). **Cierra #41** (con #48 se cierra funcionalmente el ciclo del mapa; bump de mejora). Depende de #44, #45, #47 y de **#64** (prerrequisito, ya cerrado v2.18.1). Requiere doc de requisitos. Ver §13.18/§13.19. | pendiente |
