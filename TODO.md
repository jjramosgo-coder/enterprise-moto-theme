...
| 51 | otro | **[#41 · replantea la Fase I: #42/#43/#44] Mapa interactivo con navegación continua (zoom conservando el contexto).** **Marco (contexto de partida para el Arquitecto):** #51 **replantea la Fase I**. El modelo de datos y el activo SVG en que se apoyaron #42/#43/#44 **han cambiado**; lo que aquellas fases dieron por resuelto —activos de simplemaps por país (#42), bloque que emite `europe.svg` inline (#43), motor de *swap* (#44)— **se rehace** sobre el nuevo modelo georreferenciado. **No es una mejora incremental.** **Navegación objetivo (validada por el operador):** (1) un **único SVG georreferenciado** (una proyección; nunca se cambia de fichero); (2) **zoom** animando el `viewBox` al *bounding box* del área clicada; (3) los **vecinos no desaparecen**: siguen dibujados pero atenuados (tipo `adjacent_opacity`); (4) **volver** = icono arriba-izquierda que aleja un nivel; (5) **móvil** = toque + zoom suave. Técnica tipo simplemaps, motor propio. **Habilitadores listos (preparación fuera de flujo, jul 2026):** modelo de datos georreferenciado (`map-regions-global.json`, salida de **BE Map Studio**) y el generador que produce el SVG maestro por *tiers*; viabilidad probada de punta a punta; documentado en el **manual de usuario** (`interactive-maps/manual-enterprise-maps.md`). El modelo vigente **sustituye a `region-codes.json`** y deja obsoletos `map-regions.json`/`map-levels.json` (en `claude/res/deprecated/`). **Pendiente (implementación en el tema):** llevar un **SVG maestro** al tema como activo versionado; **reescribir `assets/js/region-map-frontend.js`** (de *swap* → un único SVG + capas por `tier` + zoom de `viewBox` + atenuar vecinos + icono volver); cablear `render.php`/encolado. **A arbitrar en su doc de requisitos:** tratamiento de **islas** (flag del generador), **países-contexto** e **idioma** del nombre bilingüe. El clic en la hoja (redirect a entradas filtradas) es **#46**, no #51. **Requiere documento de requisitos** (vuelve al flujo Arquitecto→Desarrollador). Ver §13.19 y el manual en `interactive-maps/`. | pendiente |
