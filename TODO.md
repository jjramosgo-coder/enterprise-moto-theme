...
| 46 | mejora | **[#41 · Fase III.1] Página de destino por región (plantilla dedicada + dos carruseles).** El clic en la unidad **terminal** del mapa lleva a una **página de destino por región**, realizada con una **plantilla dedicada que recibe el `region_code` desde la URL** (el enlace del globo lo transporta) — **no** el archivo nativo del término, porque la taxonomía `regiones` se registró **sin parte pública** (Decisión D de #45). La página muestra **dos carruseles horizontales**: (1) **etapas** (Tipo B/C) que cruzaron la región —entradas del término `regiones` de la unidad terminal, **misma fuente** que el `count`/`data-count` de #57: el número del globo coincide exacto— y (2) **entradas Tipo D** que **contienen** alguna de esas etapas (por el filtro de viaje) —lista aparte, cuenta propia; nunca páginas «Colección de viajes»—. Modelo **A+B** (operador, Fase 4 de #45): **disuelve el descuadre de contador**. **Interfaz de #45:** dado el `region_code`, #45 entrega el conjunto de etapas publicadas (+conteo); la **derivación de los viajes** (carrusel 2) y el **montaje de la página** son trabajo de #46. Respeta el contrato de navegación (§6/§13.1) con un **nuevo contexto de origen `from_region`** integrado en `enterprise_nav_origin_params()` y el arrastre de ancestro (§13.11/§13.13). Reutiliza la card `.trip-card` (`enterprise_trip_card_data`) y el carrusel compartido (`carousel.*`); el salto desde el globo es **análogo** al «→ Entradas relacionadas» de `routes-by-location` (referencia de navegación, **no** reutilización del motor). **Agregación etapa→viaje (Tipo D): RESUELTA** como **entradas Tipo D** que contienen la etapa (por el filtro de viaje `_post_viaje_*`); el modelo previo de «pertenencia a colecciones «Colección de viajes»» quedó **ANULADO** (corrección 2026-08, verificada en código: `regiones` se asigna solo a Tipo B/C; las «Colección de viajes» agrupan Tipo D y la única, «Salidas de un día», es Tipo C, cuya etapa ya sale en el carrusel de etapas). Depende de #45 y del Bloque I. Requiere doc de requisitos. Ver §13.19 («Modelo de consumo aguas abajo»). **Prerrequisito #64: RESUELTO** (el globo reaparece en las terminales tras el zoom; v2.18.1). | pendiente |
