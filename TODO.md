...
| 58 | mejora | **[#45 · Fase 1 · base] Par mapa+árbol al almacén del sitio (subida y validación como par; `render.php` lee del almacén).** Materializa la Decisión B (Opción 2) de #45: el **par** `enterprise-eu.svg` + `map-regions-global.json` se **sube junto** desde la pantalla «Enterprise Moto» (Decisión A) al **almacén del sitio** (`uploads`), validado de forma **transaccional** (los dos o ninguno) + esquema de cada uno + misma hornada si el generador los sella (**sin** paridad nodo-a-nodo: respeta el async código↔geometría §13.19/#53). `blocks/interactive-region-map/render.php` (hoy lee `get_template_directory().'/assets/maps/enterprise-eu.svg'`, l. 22) pasa a leer el SVG del **almacén**, con la copia del tema como **semilla/respaldo**. **Acto de cierre documental:** corregir la instrucción del proyecto y §13.19 que hoy llaman al mapa «activo de git» → el contenido del mapa vive en el almacén del sitio (sigue sin bump). **Base sobre la que se apoya #55.** Depende de #45 (diseño). Requiere doc `requirements-58-*`. Ver §13.19. | pendiente |
