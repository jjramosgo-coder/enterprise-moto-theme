...
| 17 | mejora | **Bloque nuevo de mapa de localizaciones enlazadas a rutas filtradas** (mejora sustancial; toma `location-map` solo como referencia — el actual se mantiene). Cada marcador (localización) guarda, además de nombre y coordenadas, un **filtro compuesto** sobre las taxonomías existentes: **(cat_1 OR … OR cat_n) AND (tag_1 OR … OR tag_m)** (vocabulario `_filt_*`/§7, reutilizando `enterprise_stage_query()`, `functions.php:388`). El popup enlaza a un **destino que auto-compone el grid** de las entradas que casan ese filtro (incluye etapa/jornada/viaje sin re-etiquetar), reusando el render de archivo; el caso degenerado de **un solo término** es el archivo nativo (`get_term_link()`). Alta de localizaciones con el **buscador Nominatim** ya existente (nombre → coordenadas) **más clic en el mapa**; gestión de decenas/cientos mediante **Modal** (contenedor) con **mapa OpenLayers + DataViews/DataForm** (WP 7.0), sustituyendo la lista de la barra lateral que no escala. Almacenamiento **por-bloque** (atributos); reutilización = copiar el bloque (copy/paste conserva marcadores). **Fuera de alcance inicial:** contexto de navegación prev/next del destino (§6 — TO-DO aparte) y el **mecanismo de enrutado/URL del destino compuesto** (su propio spec; requiere confirmar antes qué es `/las-rutas/`). Requisitos: `claude/requirements/requirements-17-<slug>.md` (por redactar). | pendiente |
