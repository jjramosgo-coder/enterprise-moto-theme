...
|---|------|-------------|-------------|
| 65 | fix | **[#46 · follow-up] Bucle «Volver» viaje↔etapa en el carrusel de viajes de la página de región — CERRADA.** El carrusel 2 (Tipo D) hacía **salto plano** (§3.5 de #46) → el viaje llegaba sin contexto, su «Volver» caía a `wp_get_referer()` y el ida-y-vuelta a una de sus etapas lo volvía **bucle viaje↔etapa**. Fix: el carrusel 2 **estampa `from_region`** en la card de viaje igual que el de etapa (**reabre y sustituye §3.5 de #46**); `single.php` **bifurca el prev/next por tipo** (viaje → viajes **publicados** de la región vía `enterprise_region_trip_ids` filtrado a `publish`, orden del carrusel 2; etapa → etapas del término, sin cambio), etiquetas «Viaje anterior / Siguiente viaje» para el viaje, y **«← Volver a la región» unificado** para etapa Y viaje. Sin tocar `enterprise_nav_origin_params()` ni `post-stages/render.php` (ya arrastran `from_region`). Precedente: `from_col` (#8). Implementado por el Desarrollador en `c90928c` (`template-region-destino.php`) + `d3a833d` (`single.php`), validado por Juanjo en WordPress real. Requisitos: `claude/requirements/closed/requirements-65-bucle-volver-viaje-region.md` (no versionado). Ver §13.19/§13.20. | §13.19 + §13.20 del design doc + **bump 2.22.2 → 2.22.3** (ago 2026) |
| 66 | mejora | **[#41 · deriva de #48] Afordancia de acción del globo-enlace y su alcance táctil — CERRADA.** Plan de 3 fases secuenciales, versión en 2.22.X: **66.1** separar ratón/táctil (Pointer Events por `pointerType`; `352295e`+`8885d73`, sin bump); **66.2** navegación con ratón del último tier (globo sigue el cursor + «Ver las rutas» + clic en la región navega; globo etiqueta no clicable; `41d25a6`+`762be2e`+`2b71998`, v2.22.1); **66.3** rediseño de la navegación **táctil** de **dos toques** (1.er toque revela + acento por clase `.ent-touch-accent`; 2.º confirma por tipo de pieza: drillable→entra / terminal con rutas→navega / terminal sin rutas→no-op; `touch-action:manipulation`; globo `pointer-events:none`; reconcile de #64 suprimido en táctil con la rama de ratón intacta; **desviación autorizada**: acotar el acento `:hover` a `@media (hover: hover)` por el `:hover` **pegajoso de iOS** que doble-acentuaba; `f33c5a6`+`7664261`+`8ba4578`, v2.22.2). Validado por Juanjo en WordPress real (incl. iPhone). Requisitos: `claude/requirements/closed/requirements-66-1/2/3-*.md` (no versionados). Follow-ups menores opcionales: limpiar el `click` inerte del globo (motor ~l.211) y acotar el `:hover` del botón «volver» (cosmético). Ver §13.19. | §13.19 del design doc + **bump 2.22.1 → 2.22.2** (ago 2026) |
