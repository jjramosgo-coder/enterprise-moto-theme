...
| 38 | mejora | **Composición de dos capas para el *slot* vacío de «Próxima expedición» (`.next-exp`).** Cuando no hay destino/imagen de ruta definidos, sustituir el placeholder actual (`.next-img-placeholder`, el emoji 🗺️ + texto, `functions.php` l. 3397-3402) por una composición de dos capas dentro de la misma caja **4/3** ya existente: capa de fondo con un **mapa semitransparente** y, superpuesta y centrada, una **imagen** (la brújula de marca). **Técnica recomendada:** contenedor `display:grid` con ambas capas en la misma celda (`grid-area:1/1`), sin `position:absolute`; mapa `object-fit:cover` a pantalla completa de la caja, imagen superior centrada con `place-self:center` y ancho ~55%. Alternativa descartada: `position:relative` + capas `absolute`+`transform` (mete posicionamiento absoluto innecesario). **Principio:** la **semitransparencia va en CSS** (`opacity` sobre la capa del mapa o tinte `rgba(14,14,14,…)` encima), **no horneada en el fichero** — el activo se mantiene opaco y reutilizable, coherente con el fondo oscuro editorial. **Pautas de formato/peso (arquitectura):** brújula → **SVG** (vectorial, nítida a cualquier tamaño, peso despreciable); mapa fotográfico → **WebP** (evitar PNG para fotos), 1200-1600 px de ancho para cubrir retina 2×, **< 150-250 KB**, compresión agresiva admisible al ir tenue; mapa vectorial simple → SVG (rasterizar a WebP si supera ~100-150 KB). **Rendimiento:** conservar `aspect-ratio:4/3` fijo (evita *layout shift*, ya presente); `loading="lazy"`+`decoding="async"` en el mapa raster si queda bajo el pliegue; no apilar dos rásters pesados. **Alcance:** solo el estado vacío del *slot*; con imagen de ruta configurada (`enterprise_next_image`) se sigue mostrando `.next-img`. CSS en línea del bloque en `functions.php` (l. 3242-3251). **Requiere documento de requisitos** para el Desarrollador (cambio de comportamiento visual). El disparador exacto del estado vacío (ausencia de `enterprise_next_image` vs. de `enterprise_next_title`) se concreta al escribir el requisito. Surgido en sesión de arquitectura jul 2026. | pendiente |
| 39 | mejora | **Agrupar los bloques propios bajo una categoría con nombre en el inserto del editor.** Hoy los bloques `enterprise/*` se registran en `functions.php` (`register_block_type`) **sin clave `category`**, y el tema **no registra ninguna categoría de bloques** (`block_categories_all`); por eso caen en «Sin categoría», al final de la pestaña Bloques. Objetivo: registrar una categoría de bloques propia (slug `enterprise-moto`, etiqueta **«Enterprise Moto»**, el nombre del tema) y asignarla a todos los bloques `enterprise/*` para que aparezcan agrupados y **en primer lugar**. Ojo: la categoría «Enterprise Moto» ya existente es de **patrones** (`register_block_pattern_category`), no de bloques — distinto registro; aquí se crea la de bloques. Requiere documento de requisitos (toca el registro PHP y el `category` del `registerBlockType` del editor). **Alimentado por #43:** debe incluir también `enterprise/interactive-region-map` (alta en #43) en la asignación de la categoría. Surgido en sesión de arquitectura jul 2026. | pendiente |
| 40 | doc | **Documentar y aprovechar los patrones de bloque (block patterns) del tema.** (1) *Documentación:* el mecanismo de patrones y el único patrón existente —«Carrusel de etapas de ruta», registrado **programáticamente** con `register_block_pattern` (`functions.php` l. 2025) bajo la categoría de patrones «Enterprise Moto»— **no constan** en el documento de diseño (revisadas §6, §7, §8); documentarlos (sitio natural §7 «Bloques Gutenberg», o donde el Arquitecto decida): qué es, cómo se registra, cómo se usa desde el inserto y por qué no aparece en «Mis patrones» (esos son `wp_block` de la BD, dato de instancia). (2) *Aprovechamiento:* valorar cómo explotar los patrones como elemento de arquitectura de WP (composiciones listas de bloques propios) y si conviene añadir más; registrar la valoración como decisión de arquitectura. Si concluye construir patrones nuevos, cada uno sale como su propio TODO `mejora`. Surgido en sesión de arquitectura jul 2026. | pendiente |
| 50 | (sin clasificar) | **[#41 · deriva de #44] Accesibilidad de teclado + lector de pantalla del mapa de regiones.** Retirada deliberadamente de #44 por decisión del operador («se navega con el ratón y punto»). Añadir navegación por teclado (foco sobre los `<path>` navegables, activación con Enter/Espacio) y semántica ARIA (nombre accesible, anuncio del cambio de nivel) sobre el motor `region-map-frontend.js`. Desciende de #44. Requiere doc de requisitos. Ver §13.19. | pendiente |
| 59 | mejora | **[#45 · Fase 1 · mejora acotada] Migración de posts entre términos de `regiones` desde el reporte de divergencias.** Acción **deliberada del operador** para el caso raro de un **cambio de código ISO** (p. ej. `ESNA→ESNC`): mover las asignaciones de los posts del término viejo (descolgado CON posts) al nuevo, disparable desde el reporte de #55. No es efecto de «Sincronizar» (que es no destructivo, Decisión F). Depende de #55. Requiere doc `requirements-59-*`. Ver §13.19. | pendiente |
| 60 | mejora | **[#45 · #56 · follow-up diferido] Persistir `useGeoInventory=off` cuando los metadatos de un bloque no son `VALID` + aviso no bloqueante en el editor.** Diferido del Commit 6 de #56 (Decisión B del operador, 2026-08-02) por delicado y no necesario para la corrección de la ingesta. **(1)** Al publicar, si el metadato de un bloque `route-metadata-map` no es `VALID`, poner su atributo `useGeoInventory` en **off persistido** (reescribir el atributo en `post_content`: parse→flip→serialize→`wp_update_post`, con guarda de reentrada; riesgo de descolocar otros bloques). **(2)** Aviso no bloqueante en el editor («Este bloque no alimenta el inventario del mapa de regiones: los metadatos geográficos no son válidos…»), atado a (1). **No bloqueante:** la ingesta ya re-chequea `VALID` en servidor cada vez → el conteo es correcto sin esto. **Decidir si se implementa.** Depende de #56. Requiere doc `requirements-60-*`. Ver §13.19/§8. | pendiente |
| 61 | doc | **[#45 · #56 · marca] Documentar los tokens de color funcionales nuevos del espejo de estadísticas.** Al corregir el espejo del bloque `route-metadata-map` (#56) para respetar el mockup aprobado, se dieron de alta en `:root` de `style.css` tokens **funcionales** de color: la escala de pendiente `--slope-1/2/3` (#85B7EB / #378ADD / #185FA5) y el par rojo `--red-tint` / `--red-dk` (#f7e4e1 / #8a2820), usados en `assets/css/route-metadata-map.css`. Documentarlos en el **manual de marca** (§3.2 «Paleta Editorial»), **clasificar marca vs funcional** (§13.18) y decidir si `--red-*` entra en la Paleta Editorial. **Lo ejecuta Branding Copilot.** Surgido en #56 (sin subir versión, regla de #45). | pendiente |
| 65 | fix | **[#46 · follow-up] Bucle «Volver» viaje↔etapa en el carrusel 2 (Tipo D).** El salto del carrusel 2 (entradas Tipo D) es **plano** (§3.5): el viaje no recibe contexto de navegación → `single.php` cae a `wp_get_referer()`; al entrar en una etapa del viaje (`from_post`) y volver, el referer del viaje pasa a ser la etapa → **ping-pong**. Raíz: §3.5 (salto plano, sin contexto) × fallback por referer preexistente en `single.php`. Vías (a decidir por el Arquitecto al abordarlo): (a) estampar `from_region` también en la card del Tipo D —contradice §3.5, reabrir—; (b) cambiar el «Volver» sin contexto. Inclinación preliminar (IA): vía (b), para no reabrir §3.5. Ref. cruzada con #46. Requiere doc de requisitos. Surgido en el cierre de #46 (ago 2026). | pendiente |
| 66 | mejora | **[#41 · deriva de #48 · mejora menor de UX, no bloqueante] Afordancia de acción del globo-enlace y su alcance táctil.** **Plan de ejecución (3 fases secuenciales; cada fase: diseño → `requirements-66-<fase>-*` → implementación → validación en WordPress → cierre + §13.19; la versión se mantiene en 2.22.X):** **· 66.1 Separar ratón/táctil, sin cambio de comportamiento (HECHO — `352295e`+`8885d73`, validado, sin bump; §13.19)** — desdoblado el camino único de eventos de ratón en dos por `pointerType` (**Pointer Events**, estándar maduro), cada input haciendo lo que hacía; **habilitador** de 66.2 y 66.3. Requisitos en `claude/requirements/closed/requirements-66-1-*`. **· 66.2 Resolver las carencias de la navegación con ratón (HECHO — `41d25a6`+`762be2e`+`2b71998`, validado, v2.22.1; §13.19)** — con el ratón, en el último tier con rutas el globo **sigue el cursor** y muestra «Ver las rutas», y el **clic en la región** navega (globo etiqueta no clicable); táctil intacto. Requisitos en `claude/requirements/closed/requirements-66-2-*`. **· 66.3 Repensar la navegación táctil (pendiente)** — el grueso de UX; incluye el arbitraje de interacción (a/b/c); la asignación fina de las acciones (3)/(5) y del estilo `.is-link` a 66.2 vs 66.3 se fija al abrir cada fase. **Detalle heredado (acciones, arbitrajes y punto de partida — sin cambios):** Extrae de #48 las acciones (3) pista de acción y (5) uso táctil + anclaje, por estar acopladas (la invitación a actuar y su alcanzabilidad con el dedo son el mismo problema). **(3) Pista de acción:** el globo invita a ver el contenido **solo** en las piezas terminales con `data-count>0`, con texto tipo «**Ver las rutas**»; sin pista «acercar» (el gesto es evidente). **(5) Uso táctil por nivel + anclaje.** Punto de partida verificado en el motor (`region-map-frontend.js`): hoy solo escucha eventos de **ratón** (`mouseover`/`mousemove`/`mouseout`/`click`) y el dedo funciona por **emulación sintética** del navegador sobre ese mismo `click` → ratón y táctil están **acoplados**; separables con **Pointer Events** (`event.pointerType`). Restricción física: la pantalla táctil **no tiene *hover*** (el «pasar por encima» del ratón se resuelve con otro gesto, no se replica). A arbitrar en sesión: **(a)** modelo de gesto táctil (un toque actúa vs. primer toque revela / segundo confirma), **consistente por nivel** para que el enlace sea alcanzable; **(b)** convertir el toque **no-op** de la provincia hoja con rutas en «revela su globo-enlace»; **(c)** dónde se ancla el globo al tocar el último nivel (punto tocado / centro de la pieza / posición fija). **No bloquea el cierre de #48 ni de #41.** Deriva de #48; se apoya en #64 (ya cerrado v2.18.1). Requiere **sesión de arbitraje antes de requisitos**, y doc `requirements-66-*`. Ver §13.19. | pendiente |
| 67 | doc | **[#48 · marca] Documentar el token funcional `--balloon-fg` (#f5f5f5).** Al tokenizar el color del globo del mapa (#48) se dio de alta en `:root` de `style.css` el token **funcional** `--balloon-fg: #f5f5f5` (texto del globo `.ent-region-map__balloon`). Documentarlo en el **manual de marca** / doc de tokens y **clasificar marca vs funcional** (§13.18), análogo a **#61** (tokens del espejo de #56). **Lo ejecuta Branding Copilot.** Surgido al cerrar #48 (ago 2026). | pendiente |

## Resueltas

| # | Tipo | Descripción | Resuelto en |
|---|------|-------------|-------------|
| 41 | mejora | **Nuevo bloque «Mapa interactivo de regiones» — CERRADO.** TO-DO **raíz** del plan del mapa: choropleth **SVG DOM-only** (motor propio, sin OpenLayers), navegación por niveles con zoom del `viewBox`, colores configurables por el autor, capa de datos de conteo/filtrado por región y página de destino por región. Plan diseñado jul 2026 (§13.19) y ejecutado por fases **#42–#48** (+ derivados #51/#54/#57/#64). Se **cierra funcionalmente con #48** (Fase A del globo). Derivados abiertos **no bloqueantes**: **#50** (accesibilidad teclado/lector), **#66** (afordancia de acción + táctil del globo), **#59/#60/#61** (capa de datos). Ver §13.19 (mecanismo, decisiones y traza de cierre fase-a-fase). | §13.19 del design doc + **cierre en v2.22.0 con #48** (ago 2026) |
| 48 | mejora | **[#41 · Fase III.3] Globo de hover completo — CERRADA (Fase A).** Entregó el globo del mapa en **contenido y estilo**: **nombre** en español (ya existente, verificado), etiqueta «**Rutas: N**» delante del conteo (`data-count>0`; `0`/ausente sin línea) y **tokenización del color** del globo (§13.18: sin literales en `.ent-region-map__balloon`; token funcional nuevo `--balloon-fg: #f5f5f5` en `:root`). Las acciones **(3)** pista de acción «Ver las rutas» y **(5)** uso táctil por nivel + anclaje se extrajeron al spin-off **#66** (mejora menor de UX, no bloqueante). Implementado por el Desarrollador en `f6d21a8` (etiqueta, siempre plural) + `e090293` (tokenización, HEAD), validado por Juanjo en WordPress real. Requisitos: `claude/requirements/closed/requirements-48-globo-fase-a.md` (no versionado). Follow-up de doc: **#67** (documentar `--balloon-fg`). Ver §13.19. | §13.19 del design doc + **bump 2.21.0 → 2.22.0** (ago 2026) |
| 47 | mejora | **[#41 · Fase III.2] Presentación del mapa: realce por conteo, ancho de columna e higiene del SVG — CERRADA.** Realce por `data-count`: atenuación al 50 % de las regiones sin etapas (`0`/ausente) por canal de pintura (`fill-opacity`/`stroke-opacity`, multiplica con la opacidad de #54 y el atenuado del motor; acento de hover a la mitad); hover-lift **CSS** de las regiones con etapas (opción A; opción B —reordenar en el motor— documentada, no usada). El mapa **llena la columna** (retirado `max-width:900px`; `wide`/`full` efectivo — **absorbe #62**). `render.php` retira el prólogo `<?xml?>` con **activo inviolable (D-1)** (**absorbe #63**). **Adenda estética (arbitraje del operador):** etiqueta de la unidad enfocada (**tocó el motor** `region-map-frontend.js`) y monograma de marca (`<img>` decorativo servido por `render.php`). Implementado por el Desarrollador en `5e7ee43`/`34cad5d`/`3897822`/`0b29dc7` + `0443359`/`1ce5ed6`, validado por Juanjo en WordPress real. Requisitos: `claude/requirements/closed/requirements-47-presentacion-mapa.md` (con §7 adenda). Ver §8 y §13.19. | §8 + §13.19 del design doc + **bump 2.20.0 → 2.21.0** (ago 2026) |
| 62 | fix | **[absorbido en #47] El mapa interactivo de regiones no llena la columna de contenido.** `.ent-region-map { max-width: 900px }` (`interactive-region-map.css`) topa el mapa a 900px < ~1160px de columna útil y **neutraliza** el `align` wide/full que el bloque ya declara. Fix de presentación (CSS): retirar el tope (y el `margin-inline:auto` inerte), conservar `position:relative` (ancla «volver»); doc `claude/requirements/requirements-62-ancho-mapa-regiones.md` ya redactado. | **Cerrado por absorción en #47** (2026-08-03): su ejecución (código, validación en WordPress y bump) se consolida al cerrar **#47**, no como fix aislado; el plan previo de bump 2.18.0→2.18.1 queda superado. Conserva su `#`. Ver #47 y §13.19. |
| 63 | limpieza | **[absorbido en #47] Prólogo `<?xml version="1.0"?>` en el SVG emitido.** El activo `enterprise-eu.svg` empieza por ese prólogo y `render.php` lo emite tal cual (`echo $svg`); en HTML5 se parsea como **comentario bogus** (no es texto visible) pero es marcado inválido: retirarlo (en `render.php` antes del `echo`, o del propio activo), a acotar al redactar el requisito. Clasificado `limpieza` (era «sin clasificar»). | **Cerrado por absorción en #47** (2026-08-03): su ejecución y validación se consolidan al cerrar **#47**. Conserva su `#`. Ver #47 y §13.19. |
| 46 | mejora | **[#41 · Fase III.1] Página de destino por región (plantilla dedicada + dos carruseles) — CERRADA.** Plantilla dedicada `page-templates/template-region-destino.php` (`?region=<code>` → término `regiones` por `region_code`; **Opción A**, `regiones` sigue `public=false`; **Decisión D reabierta y confirmada**), elegida en el Customizer (`enterprise_region_dest_page`) con URL de `enterprise_region_destination_url()`. Dos carruseles: (1) **etapas** del término (`enterprise_region_stage_query`, fuente única compartida con el prev/next de `from_region`); (2) **entradas Tipo D** que las contienen (`enterprise_region_trip_ids` sobre `enterprise_viaje_stage_ids`; **modelo «Colección de viajes» ANULADO** — corrección `43ab870`), caché en el transient `enterprise_region_trips_map` (rebuild perezoso, invalidado en `enterprise_post_stage_save`). **Hero:** nombre + nº etapas + nº viajes + subtítulo «Rutas que cruzaron esta región» + ticker `_post_ticker_name` + nombre bilingüe. **`from_region`** (§6/§13.13): prev/next sobre la secuencia del término, «Volver» a la región, «← Volver al mapa» por `region_src`; estampado solo en las cards de etapa; carrusel 2 (Tipo D) salto plano. **Globo-enlace:** enlace en el globo activo solo si `data-count>0`, en terminal ancla y navega, y muestra el nº de entradas (número a secas; el globo completo es #48). Implementado por el Desarrollador en `09ccbc1` (plantilla+enrutado+Customizer+helper), `10ceab0` (carrusel Tipo D + caché), `8453caf` (globo-enlace+conteo), `a6a4117` (`from_region`), `bbd2a57` (fix hero) + corrección de modelo `43ab870`; validado por Juanjo en WordPress real. Requisitos: `claude/requirements/requirements-46-pagina-destino-region.md` (no versionado). Follow-ups: **#48** (UX táctil del globo-enlace + globo completo), **#65** (bucle «Volver» viaje↔etapa). Invariante duro: solo Tipo B/C. Ver §13.19. | §13.19 del design doc + **bump 2.19.0 → 2.20.0** (ago 2026) |
| 45 | mejora | **[#41 · Fase II · Bloque II] Capa de datos de conteo/filtrado por región — CERRADA (5 fases).** Dio al tema una **capa geográfica consultable** que relaciona las etapas (Tipo B/C) con las unidades del mapa. Materializada como **taxonomía plana `regiones`** (join por `region_code` = `id` del `<path>`; asignación de ancestros para contar cada nivel), **sembrada/actualizada** idempotente y no destructiva desde el árbol autoritativo con la herramienta **«Sincronizar regiones»** (#55), sobre el **par mapa+árbol en el almacén del sitio** subido y validado desde la página «Enterprise Moto» (#58). **Asociación e ingesta** por el bloque **«Mapa de ruta con metadatos»** (`enterprise/route-metadata-map`, #56), con **puerta de publicación** de completitud (lo geográfico **no bloquea**; ingesta condicional a `VALID`, por unión/reemplazo). **Conteo** leído del core e **inyección aditiva de `data-count`** por región en el render (#57). **Consumo aguas abajo (Fase 4, diseño):** destino del clic = página por región con **plantilla dedicada** (`region_code` en URL) + **dos carruseles** (etapas del término / viajes «Colección de viajes» que las incluyen) + contexto `from_region`; su ejecución es **#46**. Fronteras: **#48** pinta el conteo en el globo, **#46** monta la página. **Invariante duro:** solo Tipo B/C. Documentado en **§13.19**. Sub-hitos: #58/#55/#57 **sin bump**, #56 → 2.18.0 (excepción); cierre del hito → **2.19.0**. Follow-ups no bloqueantes: #59 (migración de posts), #60 (persist-off), #61 (doc de tokens, Branding); #64 (globo terminal) ya cerrado. | §13.19 del design doc + **bump 2.18.1 → 2.19.0** (ago 2026) |
| 64 | fix | **El globo de nombre no reaparecía en las regiones terminales tras el zoom.** Solo se encendía en `mouseover`; una región **terminal** (un clic, sin sub-nivel) dejaba el cursor sobre la misma pieza tras el zoom → sin `mouseover` nuevo, no volvía (y en táctil no aparecía). Arreglo **JS-only** en `region-map-frontend.js`: **reconciliación del globo en el `onDone` de `animateVB`** (cubre país/región/terminal/volver/Europa) según el último punto del puntero/toque (`ptrX/ptrY`); en **táctil** se muestra en el punto tocado y **persiste hasta el siguiente toque**. **Desviación de la spec (documentada):** en enfoque a **pieza única** (rama terminal) se muestra el nombre **determinista** de la pieza (`pendingBalloonPath`), no el hit-test —que en táctil caía sobre el vecino del margen (Navarra→«Nueva Aquitania»)—; `elementFromPoint` queda solo para volver/Europa y drills a sub-capa. Implementado por el Desarrollador en 1 commit (`dc562ca`), validado por Juanjo (desktop y táctil). Requisitos: `claude/requirements/closed/requirements-64-globo-terminal-tras-zoom.md` (no versionado). **Prerrequisito de #48/#46.** Ver §8 y §13.19. | §8 + §13.19 del design doc + **bump 2.18.0 → 2.18.1** (ago 2026) |
| 57 | mejora | **[#45 · Fase 3] Inyección de `data-count` por región en el render.** El `render.php` de `interactive-region-map` inyecta `data-count` en cada `<path>` desde el `count` del término `regiones` (join `path.id → region_code`), por **inyección aditiva** (`preg_replace_callback`) con **tres estados** (con término → `data-count="N"` incl. `0`; sin término → sin atributo). Conteo leído del core (`get_terms hide_empty=false`), cacheado en el transient `enterprise_regiones_counts` (TTL 12 h) e **invalidado** por `edited_term_taxonomy`/`set_object_terms`/`created_term`/`delete_term` (acotados a `regiones`). **Frontera con #48 fijada:** #45 deja el dato en el DOM; #48 lo pinta. **Basado en conjuntos.** Implementado por el Desarrollador en 2 commits (`20516b2` helper+inyección, `f0ea209` caché transient+invalidación por hooks) y validado por Juanjo en WordPress real (incremento/decremento al publicar/despublicar; alta/baja de término vía «Sincronizar»). Requisitos: `claude/requirements/closed/requirements-57-inyeccion-data-count-region.md` (no versionado). **Depende de #55/#56; alimenta #48/#46.** Ver §8 y §13.19. | §8 + §13.19 del design doc; **sin bump** (consolida al cierre de #45); base v2.18.0 |
| 56 | mejora | **[#45 · Fase 2] Bloque «Mapa de ruta con metadatos» (`enterprise/route-metadata-map`) + ingesta y puerta de publicación.** Alternativa a `route-comparison` que **además** carga un fichero de metadatos por entrada: dibuja planificada (opcional)+registrada reutilizando `map-frontend.js`, muestra el **espejo completo** de las métricas del metadato (con botones «info» accesibles) y **alimenta la taxonomía `regiones`**. Editor botón→Modal (fecha + `trip` informativo + tres cargas locales), ciclo de vida de dos estados, barra lateral heredada + interruptor `useGeoInventory`. Almacenamiento `uploads/routes/recorded/<year>/<month>/` (sufijo `(N)` antes de la extensión, persistido en `assetSuffix`) vía endpoint REST `enterprise/v1/route-metadata` (POST validado por hash/duplicidad, DELETE). **Puerta §4.4:** bloquea **solo por completitud** (no por lo geográfico) vía `rest_pre_insert_post`→`WP_Error` + `lockPostSaving`; **ingesta geográfica no bloqueante** (solo Tipo B/C) por unión/reemplazo condicionada a `VALID`. **Mejora compartida:** altitud en el cursor del perfil (`map-frontend.js`, co-beneficia a los demás bloques de mapa). Implementado en `bffe645`,`e4e064d`,`5bc87d5`,`aaa1658`,`1d82214`,`4eb3cc6`,`dd13a66`,`ed530d3`; validado por Juanjo. Requisitos: `claude/requirements/closed/requirements-56-bloque-route-metadata-map.md`. Follow-ups: **#60** (persist-off diferido), **#61** (doc de tokens, Branding). **Depende de #55/#58.** Ver §7/§8 y §13.19. | §7/§8 del design doc + **bump 2.17.0 → 2.18.0** (ago 2026) |
| 58 | mejora | **[#45 · Fase 1 · base] Par mapa+árbol al almacén del sitio (subida validada por hash; `render.php` lee del almacén).** Materializó la Decisión B/Opción 2 de #45: el par `enterprise-eu.svg`+`map-regions-global.json` se **sube junto** desde la página de admin propia **«Enterprise Moto»** (nivel superior, **solo administradores** en #58) al **almacén del sitio** (`uploads/enterprise-maps/`), con validación **transaccional** (los dos presentes + árbol con `_meta`/`tree` + sello **`data-tree-sha256`** del SVG que casa por `hash_equals` con el `sha256` de los bytes exactos del árbol; si algo falla no guarda nada; escritura por temporales+`rename`). **Resolutor** `enterprise_map_asset_path()` (almacén→tema, semilla del tema como respaldo); `render.php` l. 22 lo usa para el SVG. El sello prueba «misma generación», no paridad nodo-a-nodo (async §13.19/#53). Implementado por el Desarrollador en 2 commits (`5dca2a9` resolutor+render, `c632b53` página+subida) y validado por Juanjo en WordPress real. Requisitos: `claude/requirements/closed/requirements-58-par-mapa-arbol-almacen.md` (no versionado). **Alimenta #55.** | §13.19 del design doc; **sin bump** (consolidado con el cierre de #45, política jul 2026); base v2.17.0 |
| 55 | mejora | **[#45 · Fase 1] Capa de datos: taxonomía `regiones` + herramienta «Sincronizar».** Alta de la taxonomía propia **plana** `regiones` sobre `post`, **registrada mínima en la parte pública** (`public=false`/`publicly_queryable=false`/`rewrite=false`/`show_ui=true`; sin páginas/URLs por región — Decisión D, el destino del clic es de #46), con term meta **`region_code`** (código ISO = `id` del `<path>` = clave de join; slug = `strtolower(code)`, no portante) y **`region_admin`** (nivel). Herramienta **«Sincronizar regiones»** en la página **«Enterprise Moto»** (ampliada a **administradores + editores**; la subida del par #58 sigue solo-admin dentro de la página): siembra/actualización **idempotente y no destructiva** desde el árbol autoritativo (leído del almacén vía el resolutor de #58), recorriendo el árbol **completo** (todos los niveles, incl. nodos sin geometría — async tolerado §13.19), **emparejando por `region_code`** (E), con **vista-previa-y-confirmar SIEMPRE** (C; doble candado: «Aplicar estos cambios» solo dentro del reporte + guardia en el handler que exige el análisis previo) y **reporte de divergencias** en cuatro grupos (nuevas / nombres a actualizar / descolgadas con y sin entradas); **nunca borra términos ni reasigna posts** (término con posts protegido; «actualizar» refresca nombre **y** nivel, nunca la identidad/código — F + Opción A). La **migración** de posts entre términos es #59. Implementado por el Desarrollador en 4 commits (`aea36f9` taxonomía+meta, `0ec9fde` página a admins+editores+sección, `ccac51e` motor idempotente, `e743f3d` corrección §7 «previa siempre»/Decisión C) y validado por Juanjo en WordPress real. Requisitos: `claude/requirements/closed/requirements-55-taxonomia-regiones-sincronizar.md` (no versionado). **Depende de #58; alimenta #56/#57.** | §13.19 del design doc (párrafo propio; reconciliación del «metadato JSON» diferida a la Fase 5 de #45); **sin bump** (consolidado con el cierre de #45, política jul 2026); base v2.17.0 |
| 54 | mejora | **Colores configurables del bloque «Mapa interactivo de regiones» (rehorneo de nivel 2).** Primer bloque con `attributes`. Interruptor **«Colores del mapa»**: **«Los del propio mapa»** (colores horneados en el activo, nivel 1) / **«Personalizar»** (rehorneo por CSS del tema, nivel 2). En «Personalizar»: **paleta cerrada** en desplegable (Editorial / Enterprise, extensible) que limita los colores a los suyos (swatches tipo `ColorPalette`, sin color libre), más `stroke-width`/`opacity` numéricos por nivel. **Mecanismo (§13.19):** `render.php` emite clases de estado (`is-color-theme`, `palette-*`, `back-*`) y **custom properties inline** (color saneado con `sanitize_hex_color`, numéricos acotados); el CSS las consume con `var(--…, <valor horneado>)` → modo `asset` idéntico al previo. Selectores acotados para no romper el motor (`.ent-region-map > svg path` base; niveles por `[id="tierN"] path`, sin especificidad de id, antes del bloque de interacción para que `.ent-dimmed`/hover ganen). **Acento hover** `var(--ent-hover-accent, #c9a010)` (retira el literal `#f2c118`), por defecto también en `asset`. Botón **«volver»** en dos presets sólidos por `back-light`/`back-dark` (contraste con el lienzo), abandonando la pastilla translúcida casi ilegible sobre crema. **Fix latente de #51:** el fondo del lienzo pasó de `.ent-region-map svg` (descendiente, alcanzaba el icono del botón) a `.ent-region-map > svg` (hijo directo). Nota fiel al modelo `defaults + override por nivel`: con el activo de 3 niveles, los controles de nivel base quedan tapados por cada tier (solo «Relleno de las tierras» y los por-nivel tienen efecto visible). Implementado por el Desarrollador en 3 commits (`1107cca` editor+atributos, `3c648b1` render+custom properties, `401c1c6` hover+botón) y validado por Juanjo en WordPress real. Ref. cruzada #54↔#47 (paleta global por nivel vs color por región; orden de resolución = tema a analizar de #47). Requisitos: `claude/requirements/closed/requirements-54-colores-configurables-mapa-regiones.md` (no versionado). | §13.19/§7/§8 del design doc + bump 2.16.2 → 2.17.0 (jul 2026) |
| 53 | doc | **[datos del mapa · doc] Documentar la asincronía ISO↔geometría vectorial (huérfanos) en design doc y manual de mapas.** Los nodos del árbol sin geometría en el maestro **no son un bug**: son el problema clásico de GIS de **asincronía entre estándares** —`pycountry`/ISO 3166-2 (al día) vs Natural Earth (geometría que va por detrás)—, que aflora cuando un país reorganiza divisiones (Cerdeña 2016, regiones de Francia 2016, etc.). Reflejarlo en dos sitios: **(a) design doc** — nota para futuros desarrollos, en especial las fases que **consumen el árbol** (#45/#46/#48), de que un desfase de códigos árbol↔geometría es **esperable y no bloquea** (el motor es content-agnostic e ignora el árbol; la coherencia se garantiza aguas arriba, en la generación), para que no entorpezca; **(b) manual de mapas** (`interactive-maps/manual-enterprise-maps.md`) — explicar al usuario el problema y **cómo mantener el diccionario de alias/*crosswalk*** (`NATURAL_EARTH_ALIASES`: Natural Earth → ISO vigente, aplicado **antes del `-join`** en Mapshaper) para cuadrar los códigos no sincronizados; advertir que el diccionario de partida es **solo-Europa y no exhaustivo** y cómo ampliarlo (identificar el ISO destino del código huérfano, p. ej. de los 7 actuales solo `ESNA→ESNC` y `ESPM→ESIB` están cubiertos). Actos del Arquitecto (documentación). Surgido del diagnóstico de la asincronía (jul 2026). | **(a)** §13.19 del design doc — añadida la precisión «un `code` del árbol sin `id` en el SVG **≠** sin geometría» (colapso por alias o relleno por profundidad variable) y la definición de huérfano **real** (territorial, no léxico: zona que no dibuja nada tras aplicar alias + relleno de tiers); se advierte que contar por la resta `codes − ids` **sobre-cuenta**. **(b)** Manual de mapas — nueva subsección «Asincronía ISO↔geometría: el diccionario de alias» (problema, *crosswalk* antes del `-join`, dos tipos de desajuste —temporal vs variante de esquema/nivel—, tabla `NATURAL_EARTH_ALIASES` vigente y cómo ampliarla). El diccionario se amplió en BE Map Studio (aguas arriba) hasta cubrir los 7 casos (añadidos `ITAO→IT23`, `FR69→FR69M`, `FR75→FR75C`; los `FR67`/`FR68` eran hojas admin-3 bajo la profundidad del mapa, no huecos). Commit `docs(maps): documentar asincronía ISO↔geometría, alias NATURAL_EARTH_ALIASES y qué es un huérfano real (#53b)`. **bump 2.16.0 → 2.16.1** (con #52) (jul 2026) |
| 52 | limpieza | **[secuela de #51 · relacionado con #41] Motor de mapa content-agnostic + doc sin referencias al contenido.** Consolidado el principio «el mapa como artefacto es un asset; su **contenido** —qué países/regiones/islas aparecen— es contenido, no diseño». **(1)** El Desarrollador retiró `var OUTLIERS = { ES:['ESCN'] }` y su uso en `focusCountry`, y el parámetro `excludeIds` de `childrenBBox` (firma → `childrenBBox(state, parentId)`): motor con **cero acoplamiento a contenido**, preservando comportamiento (validado en WordPress; `grep OUTLIERS\|excludeIds` vacío). **(2)** Reescrita **§13.19** del design doc a solo mecanismo/arquitectura del modelo vigente, retirando toda referencia de contenido (Canarias, lista de países, islas, conteos por tier, `OUTLIERS`, decisiones E/F). **(3)** Añadida a las instrucciones del proyecto («Documentation and versioning») la regla: el contenido del mapa va con commit+push, **nunca bump**; el motor no lleva código específico de contenido. Requisitos (acción 1): `claude/requirements/requirements-52-motor-content-agnostic.md` (no versionado). | acción 1: commit `refactor(region-map): retirar OUTLIERS y el parámetro excludeIds…` (en repo, base v2.16.0); acciones 2/3 en design doc + instrucciones; **bump 2.16.0 → 2.16.1** (conjunto con #53, realizado jul 2026) |
| 51 | otro | **[#41 · replanteó la Fase I: #42/#43/#44] Mapa interactivo con navegación continua (SVG único georreferenciado).** Sustituyó el modelo *swap* por un **único SVG maestro** (`assets/maps/enterprise-eu.svg`, Natural Earth/dominio público, capas `tier0/1/2`, `data-admin/parent/name` bilingüe) + modelo `map-regions-global.json`; retiró los 9 SVG de simplemaps y `region-codes.json` (licencia migrada a Natural Earth). Motor `region-map-frontend.js` reescrito **DOM-only** (sin `fetch`): zoom por `viewBox` al *bbox* del nodo, vecinos **atenuados = visibles + clicables** (salto directo, incl. región↔región transfronterizo), regiones terminales (PT/AD, Ceuta/Melilla) con foco-sobre-sí-mismas, globo bilingüe, «volver» = alejar un nivel, ratón + toque. Decisiones E (islas: mantener Canarias, encuadre curado por `OUTLIERS`), F (solo 5 países, reversible), G (globo bilingüe), H (DOM-only); único acoplamiento mapa↔motor `OUTLIERS = { ES:['ESCN'] }`. #50 (teclado/lector) abierto. Requisitos: `claude/requirements/closed/requirements-51-*` (no versionados). | §13.19 del design doc + bump 2.15.0 → 2.16.0 (jul 2026) |
| 49 | mejora | **Cancelado — sin sentido con el modelo de datos del mapa.** Preveía una capa de nombres en español porque los de nivel-1 de `region-codes.json` salían en inglés (`Spain`/`France`). El **modelo de datos del mapa** (`map-regions.json` + `map-levels.json`, documentados en `manual-enterprise-maps.md`) **ya trae los nombres en español** y sustituye a `region-codes.json`, así que la premisa de #49 desaparece. Cancelado a instancia del operador (jul 2026). | cancelado: superado por el modelo de datos del mapa (jul 2026) |
| 44 | mejora | **[#41 · Fase I.3] Navegación del mapa de regiones (motor vanilla JS).** Motor propio `assets/js/region-map-frontend.js` (vanilla, sin build) sobre el SVG inline de #43; no reutiliza OpenLayers. Carga de niveles por `data-*`+`fetch` (nuevo `data-maps-base` en `render.php`; encolado del JS en `enterprise_region_map_assets()`); navegabilidad data-driven desde `region-codes.json.countries` (ES/IT/FR/PT/AD navegables, resto inerte); hover con resalte + globo de solo `name` (idioma según el activo); drill país→región→provincia con carga progresiva y caché (swap por nivel); «zoom» por `viewBox` con `getBBox`+`rAF`, apoyado en el lienzo compartido nivel-2/3 de cada país; colapso PT/AD; icono «volver» (arriba-izquierda, sin texto, sube un nivel). Implementado en 4 commits (`c33241b`, `9da7a93`, `3c95a02`, `33fe058`) y validado por Juanjo en WordPress real. **Reducción de alcance (operador):** accesibilidad de teclado/lector retirada → **#50**. **Evolución en evaluación:** el swap por país no conserva los vecinos; el zoom continuo entre países exigiría activo georreferenciado (medido: la fusión de los activos actuales no sirve) → **#51**. Requisitos: `claude/requirements/closed/requirements-44-navegacion-mapa-motor-vanilla.md` (no versionado). | §13.19 del design doc + bump 2.14.0 → 2.15.0 (jul 2026) |
| 43 | mejora | **[#41 · Fase I.2] Esqueleto del bloque `enterprise/interactive-region-map` + render estático nivel-1.** Alta del bloque (etiqueta «Mapa interactivo de regiones») en `enterprise_register_map_blocks()`: `api_version` 3, `render_callback`, `supports` (`html` false + `align` wide/full); **sin `category`** en el registro PHP y **sin `attributes`** (los por-región son #47). `render.php` emite `europe.svg` **inline** en el DOM (`<div class="ent-region-map" data-map-level="1">`), sin `wp_kses` (activo GPL propio) ni `<script>` inline — inline porque #44 necesita los nodos `<path>`. Editor: placeholder estático, `save: null`; el JS lleva `category:'enterprise-moto'` como sus hermanos pero, al no existir categoría de bloques registrada, resuelve a «Sin categoría». CSS propio (`assets/css/interactive-region-map.css`, dimensionado responsive) con **encolado condicional propio** (`has_block` + `filemtime`), **separado** de OpenLayers (no lo carga; no reutiliza su motor, §13.19). Coordinación con **#39**: se registró **sin** categoría de bloques; el bloque nuevo se agrupará al ejecutar **#39** (referencia cruzada #43↔#39; #43 alimenta #39). Implementado por el Desarrollador en 2 commits (`3b0a81d` esqueleto + render; `a6db4b4` contenedor responsive + encolado) y validado por Juanjo en WordPress real (mapa de Europa dibujado e inerte; cabe en móvil; CSS solo donde está el bloque; sin OpenLayers). Requisitos: `claude/requirements/closed/requirements-43-esqueleto-bloque-mapa-regiones.md` (no versionado). | §13.19 del design doc + bump 2.13.0 → 2.14.0 (jul 2026) |
| 42 | mejora | **[#41 · Fase I.1] Activos SVG del mapa de regiones incorporados al tema.** Los 9 SVG llevados al `assets/` versionado en `enterprise-moto/assets/maps/`: `europe.svg` (nivel-1, Robinson, 44 países ISO alpha-2, incluye `AD`), ES **con islas** (`es-regiones` 19 / `es-provincias` 52), `it-reg` 20 / `it-prov` 107, `fr-reg` 13 / `fr-prov` 96, `pt-prov` (20 distritos) y `ad-prov` (7 parroquias); contornos `pt.svg`/`ad.svg` (solo país) no llevados. Única normalización: `viewbox`→`viewBox` (1 byte/fichero; se conservan `width`/`height`; el `viewBox` correcto importa por conformidad y para la transición de `viewBox` = «zoom» de #44, no por efecto visible ahora). Comentario de copyright de simplemaps preservado in-file; atribución en `THIRD-PARTY-NOTICES.md`/`COPYRIGHT.md` (nueva fila `assets/maps/`). Lista canónica de códigos congelada en `assets/maps/region-codes.json` (`level1` 44; es/it/fr `admin1`+`admin2`; pt/ad `collapsed`/`levels`), fuente del contrato #45. Validado por Juanjo (recuentos §1.1; spot-checks ESAN→Andalucía, AD→Andorra en `level1`, AD02→Canillo; bloques OpenLayers intactos). Granularidad de commits divergente (2, no 3): `cc1cff2` (activos) y `fbe2d82` (licensing + `region-codes.json` fusionados; el mensaje solo refleja el licensing; historia no reescrita, precedente #33/#20). Nombres `level1` en inglés (verbatim de `europe.svg`); capa localizada al español → **#49** (alimenta #48). Requisitos: `claude/requirements/closed/requirements-42-activos-svg-mapa-regiones.md` (no versionado). | §13.19 del design doc + bump 2.12.0 → 2.13.0 (jul 2026) |
| 36 | doc | **Documentada la plantilla «Mapa de rutas por localización» en §6 (tabla «Plantillas de página») del design doc.** La tabla listaba solo «Cuaderno de bitácora», «Colección de viajes» y «Por defecto»; se añadió la fila de «Mapa de rutas por localización» (`page-templates/template-routes-by-location.php`) con su propósito: página-destino del bloque `enterprise/routes-by-location` que presenta las entradas del filtro compuesto de un marcador como **un carrusel de `.post-card` por categoría**; elegida en el Personalizador (`enterprise_rbl_dest_page`); cabecera «← Volver al mapa» (`rbl_src`) y contexto de navegación prev/next `from_loc`+`loc_cat`+`loc_tag`. **Consolidación** (el propósito ya constaba en §7/§8/§13.12/§13.13), sin cambio de código. **Sin bump**: corrección documental de la v2.12.0; el header del design doc se mantiene sincronizado en 2.12.0. Detectado por Juanjo (jul 2026). | §6 del design doc (sin bump, jul 2026) |
| 37 | branding | **Incorporados a la Paleta Editorial (§3.2 del manual de marca) los dos colores nuevos promovidos a MARCA por #32:** `--ink` `#3a3a3a` (texto de cuerpo sobre claro; jerarquía de tinta `--black` → `--ink` → `--mid`; contraste verificado y citado WCAG 2.1: 11,37:1 sobre blanco, 10,41:1 sobre `#f5f5f2`, AAA) y `--border-dk` `#2a2a2a` (borde marcado sobre claro, contrapartida del tenue `--border`; 14,35/13,14:1 sobre claro, ≥3:1 no-texto WCAG 1.4.11; sobre oscuro solo separador sutil). Uso **verificado en el CSS** (no de memoria): `--ink` en `.entry-content`/`.comment-content`/`.practical-info-grid`; `--border-dk` en bordes de bloques markdown/youtube. Tokens de canal `--white-rgb`/`--black-rgb`/`--gold-rgb` documentados como **notación** `rgba()` (no color nuevo); resto del set de #32 (`--shadow-rgb`, `--media-bg`, `--ink-lt`, `--tip-*`, estado, `--grad-*`, `--light-mid`) confirmado **funcional**, fuera de la paleta. Añadida decisión **§6.5** al registro del manual; rehechas las notas de §3.2 desfasadas por #32 (blanco translúcido tokenizado como `--white-rgb`, `--surface-dk` retirado, `--light-mid` revivido). Manual editado directamente (fichero no versionado, `claude/`). Cierra la condición de cierre de #32. | §3.2 + §6.5 del manual de marca (no versionado); sin bump de tema (jul 2026) |
...
### Análisis — #64 · [fix] Reaparición del globo del mapa de regiones tras el zoom

Realidad de la implementación, verificada en el cierre (clon `dc562ca`; base 2.18.0 → 2.18.1). Se conserva como traza cerrada.

**Commit (único):** `dc562ca` fix(region-map): reaparición del globo tras el zoom (desktop y táctil).

**Fichero:** `enterprise-moto/assets/js/region-map-frontend.js` (JS-only; `render.php` y el CSS del bloque sin cambios).

**Mecanismo entregado:** (a) seguimiento del puntero/toque en `state.ptrX/ptrY` (actualizado en `mousemove` y `click`); (b) `reconcileBalloonAtPointer(state)` invocado en el `onDone` de `animateVB`, por el que pasan todas las rutas de zoom (país, región, terminal, «volver», Europa); (c) en el enfoque a una **terminal** (rama terminal de `focusRegion`/`focusCountry`) la pieza enfocada se guarda en `state.pendingBalloonPath` y el reconcile muestra su `data-name` en el punto del puntero **sin hit-test**; (d) para «volver»/Europa y los *drills* que destapan sub-capa, el reconcile cae al hit-test `document.elementFromPoint(ptrX,ptrY)`→`closest('path')` con guardas `svg.contains`+`isHoverable`; (e) el `click` apaga el globo en toque a mar/vacío/pieza oculta y lo mantiene en la hoja provincia (no-op #46).

**Desviación respecto a la spec cerrada (documentada, no silenciada):** `requirements-64` §3 (decisiones 1–3) fijaba hit-test por `elementFromPoint` para **todos** los casos. En validación táctil (capturas de Juanjo) el píxel obsoleto tras el zoom de enfoque caía sobre el vecino atenuado del margen → nombre equivocado (Navarra→«Nueva Aquitania»; C. de Madrid→«Castilla-La Mancha»). Corrección aprobada por el operador: en enfoque a pieza única el nombre es determinista (la pieza que el motor ya tiene en la mano), hit-test solo sin pieza única. Reflejada en `requirements-64` §3 (decisión 5) y en §13.19 (entrada de decisión de arquitectura).

**Validación (Juanjo, WordPress real):** desktop —el globo reaparece sobre la terminal tras el zoom sin mover el ratón; país y región de tres niveles igual; hover a vecino y `mouseout` sin regresión— y táctil —toque a terminal muestra el nombre correcto; re-toque lo reenciende; otro toque lo mueve; toque a vacío lo apaga—.

**Integridad:** un único fichero tocado; `render.php` y el CSS del bloque byte-idénticos.

### Notas para documentación — #64 · [fix] (aplicadas al cerrar)

- **§8 (mapa de regiones):** párrafo nuevo «Reaparición del globo tras el zoom (#64)» — el globo reaparece al terminar cualquier zoom mostrando la pieza bajo el puntero, sin mover el ratón; en táctil persiste hasta el siguiente toque.
- **§13.19:** párrafo «Globo — reaparición reconciliada tras el zoom (#64)» (reconcile en el `onDone` de `animateVB`, `ptrX/ptrY`, persistencia táctil) + **entrada de decisión de arquitectura** «etiqueta determinista en el enfoque a pieza única; hit-test solo sin ella» + #64 añadido a la «Traza de cierre».
- **Bump** 2.18.0 → 2.18.1 en `style.css`, `ENTERPRISE_VERSION` y cabecera del design doc.
- **requirements-64** movido a `claude/requirements/closed/` y §3 actualizado (decisión 5) con el matiz de la etiqueta determinista.
