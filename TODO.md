...
| 11 | mejora | «Colección de viajes» (`enterprise/trip-collection`): que la presentación de las tarjetas sea **configurable como carrusel horizontal o timeline vertical**, como `post-stages` (reutilizando su mecanismo de `layout` y `carousel.js`/`carousel.css`), conservando la `.trip-card`; y añadir un toggle **«sin límite»** de entradas (`showAll` ⇒ `postsPerPage = -1`, sin tocar la query compartida). Corrige el fallo de diseño del arquitecto en #5 (grid en vez de carrusel/timeline). Requerimientos de desarrollo: [docs/requerimientos-11-coleccion-carrusel-timeline.md](docs/requerimientos-11-coleccion-carrusel-timeline.md). | pendiente |
