...
| 26 | otro | **Enlace del footer a la página «Licencia de contenido».** En `footer.php` (l. 54, dentro de `.footer-bottom`), el único enlace de la línea de copyright —el nombre del blog, que hoy apunta a `home_url('/')`— pasa a apuntar a la nueva página dedicada «Licencia de contenido» (reaprovechar el `<a>` existente, decisión de Juanjo; contra asumido: el nombre deja de enlazar a home). **Depende** de que la página exista con slug `licencia-de-contenido`. Requerimientos: `claude/requirements/requirements-26-footer-enlace-licencia-contenido.md` (no versionado). | pendiente |
