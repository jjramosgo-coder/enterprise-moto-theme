...
| 55 | mejora | **[#45 · Fase 1] Taxonomía `regiones` + siembra idempotente desde el árbol.** Alta de la taxonomía propia **plana** `regiones` (etiqueta «Regiones») sobre `post`, con term meta `region_code` (= `id` del `<path>` = código ISO, clave de join) y `region_admin` (nivel). **Siembra/actualización idempotente** desde el árbol JSON (`assets/maps/map-regions-global.json`, `map-regions.schema`) mediante **página de admin con botón «Sincronizar regiones»**: previsualización, reporte de divergencias, **nunca autoborra** (reporta para que el operador arbitre). Precedente reutilizable: `enterprise_ensure_cuaderno_terms` (`functions.php` l. 3196–3207). Resuelve la decisión abierta §4.5 del encargo. Cambiar las unidades del mapa es op de contenido (sin bump). **Depende de #45** (diseño) y del vocabulario de códigos (#42, hecho). Requiere doc `requirements-55-*`. Ver §13.19. | pendiente |
