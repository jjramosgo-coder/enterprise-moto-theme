...
| 29 | branding | **Imagen Open Graph de marca (previews al compartir).** Cablear `og:image` (+ `og:title`/`og:description` y Twitter Card) en `wp_head` para controlar la tarjeta de previsualización al compartir enlaces (WhatsApp, redes). Activo por defecto producido por Branding en `claude/branding/3-vigente/og/` (`og-image.png` 1200×630 + `og-image.svg` editable; lockup horizontal insignia BE + wordmark sobre `#0e0e0e`; texto trazado desde Bebas Neue). Hoy no hay `og:image`: las plataformas cogen un icono cualquiera como fallback. **Decisión pendiente de Juanjo/Arquitecto:** imagen por defecto para home/páginas sin imagen propia (este activo) y, opcionalmente, `og:image` por entrada (foto de portada de la ruta). Colocar en `assets/images/` o donde el Arquitecto decida. Requiere requirements del Arquitecto. | pendiente |
| 30 | branding | **Rediseño del footer: anclar la marca y compactar.** La columna izquierda sustituye su título de texto por el **lockup de marca** (insignia BE **con marco** + wordmark «BITÁCORA ENTERPRISE»), con el **tagline** debajo como texto HTML: «Viajar en moto por asfalto, disfrutar del camino y contarlo después» (tagline de marca adoptado, manual §2.1). Columnas derechas (SECCIONES / BLOG) y barra inferior intactas; el nombre del sitio «Diario de ruta de la Enterprise» sigue en el copyright — la marca visible de esa esquina pasa a «Bitácora Enterprise», distinta del nombre del sitio (decisión de Juanjo). Reducir el padding vertical; apilado móvil conservado; opcional en monitores anchos: marca de agua tenue del monograma. **Decisiones firmes:** (1) insignia **con marco** — decisión por-activo, no heredada del header (sin marco) ni del OG (§1.8); (2) el lockup se entrega como **SVG único de arte final** (activo de marca), no como composición HTML/CSS: el tema **solo coloca** la pieza; (3) el tagline va como texto HTML del footer, no incrustado en el SVG. **Entregables de Branding:** (a) promover el wordmark «izquierda» a arte final (Bebas Neue trazada a `<path>`, autocontenido, fondo transparente, viewBox ajustado al contenido); (b) producir la **insignia con marco** como arte final autocontenida (letras BE trazadas a `<path>`; hoy solo existe como boceto y embebida en el OG); (c) componer el **lockup de footer** como SVG arte final (insignia con marco + wordmark); (d) reglas de uso —fondo, área de respeto, tamaño mínimo— en el manual §4; iteración por ficheros `--propuesta`, validados en herramienta gráfica (§1.13/§1.14). **Integración en el tema:** por el flujo Arquitecto → Desarrollador (requirements + este TO-DO); layout/CSS y responsive = Desarrollador. Referencia visual: `claude/mockups/footer-rediseno-mockup.html`. | pendiente |
