...
**Tema WordPress:** Bitácora Enterprise v2.22.0  
...
**Globo — enlace al destino (#46; el globo completo es #48).** El enlace vive en el **globo** de hover: `render.php` emite `data-region-dest` (permalink de la Página-destino + `region_src`) solo si la página está configurada y publicada; el motor lo lee (`regionDestBase`) y, en una pieza **terminal** (`!hasChildren`) con `data-count>0`, ancla el globo y lo hace clicable (`window.location.href`), navegando a la página de la región. El globo muestra además el **nº de entradas** (`data-count`) con la etiqueta «**Rutas: N**» y estilo por tokens (§13.18), entregados por **#48** (v2.22.0); la pista de acción «Ver las rutas» y el uso táctil por nivel quedan en **#66**. Invariante uni-provincial: `count(admin-1)=count(admin-2)` por asignación de ancestros..
