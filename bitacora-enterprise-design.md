...
**Tema WordPress:** Bitácora Enterprise v2.21.0  
...
**Globo — enlace al destino (#46; el globo completo es #48).** El enlace vive en el **globo** de hover: `render.php` emite `data-region-dest` (permalink de la Página-destino + `region_src`) solo si la página está configurada y publicada; el motor lo lee (`regionDestBase`) y, en una pieza **terminal** (`!hasChildren`) con `data-count>0`, ancla el globo y lo hace clicable (`window.location.href`), navegando a la página de la región. El globo muestra además el **nº de entradas** (`data-count`) como número a secas —el tratamiento completo (etiqueta/estados/estilo) es **#48**—. Invariante uni-provincial: `count(admin-1)=count(admin-2)` por asignación de ancestros..
...
**UX del editor y globo.** El autor personaliza por región **color** (por tokens, §13.18) y **descripción** editorial; navegación **automática** (sin URL manual) y sin estado «visitado». Globo de hover: **nombre + nº de entradas relacionadas + descripción + pista de acción**, que varía por nivel («acercar» / «ver entradas»).
