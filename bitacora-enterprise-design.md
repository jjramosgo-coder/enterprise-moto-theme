...
**Tema WordPress:** Bitácora Enterprise v2.22.3  
...
| Mapa de rutas por localización | `page-templates/template-routes-by-location.php` | Página-destino del bloque «Mapa de rutas por localización» (`enterprise/routes-by-location`): presenta las entradas que casan el **filtro compuesto** de un marcador como **un carrusel de `.post-card` por categoría** del marcador, reutilizando la librería de carrusel del tema (`.ent-stages` + `carousel.js`/`carousel.css`). Se elige en el Personalizador (control `enterprise_rbl_dest_page`) y la URL por marcador la deriva `enterprise_rbl_destination_url()`. Cabecera «← Volver al mapa» (param `rbl_src`) y contexto de navegación prev/next propio `from_loc`+`loc_cat`+`loc_tag`. Alta en #17 (v2.8.0); rediseño definitivo del destino en #18 (v2.9.0). Ver §7 (catálogo de bloques), §8 (sistema de mapas) y §13.12/§13.13. |
...
| Mapa interactivo de regiones | `enterprise/interactive-region-map` | **Choropleth vectorial**: regiones administrativas como `<path>` de un **SVG maestro único georreferenciado** emitido inline; **no usa OpenLayers ni tiles/coordenadas** — paradigma alternativo y complementario a los mapas OSM (§8, §13.19). Navegación continua por niveles (`tier0/1/2`) con zoom por animación del `viewBox`; vecinos visibles y clicables. Motor propio **DOM-only** `region-map-frontend.js` (vanilla, sin build, sin `fetch`: jerarquía y nombres leídos de los `data-*` del propio SVG). **Colores configurables por el autor** (#54): interruptor «Colores del mapa» —«Los del propio mapa» (horneados en el activo, nivel 1) / «Personalizar» (rehorneo por CSS del tema, nivel 2)—; en «Personalizar», **paleta cerrada** en desplegable (Editorial / Enterprise, extensible) que limita los colores usables, más grosor y opacidad por nivel; **acento hover** propio (`#c9a010` por defecto) y botón «volver» con preajuste claro/oscuro. El **fondo del lienzo NO es configurable** (invariante §13.19). Alta del esqueleto y render en #43 (v2.14.0); navegación en #44 (v2.15.0); Fase I replanteada al maestro georreferenciado único en #51 (v2.16.0); colores configurables en #54 (v2.17.0). Registrado **sin categoría de bloques** (cae en «Sin categoría» hasta #39). Ver §8 y §13.19. |
| Mapa de ruta con metadatos | `enterprise/route-metadata-map` | Alternativa a `route-comparison` que **además** carga un **fichero de metadatos** por entrada (portador de la asociación entrada↔región de #45). Dibuja la ruta **planificada (opcional)** y la **registrada** reutilizando el motor `map-frontend.js` vía `data-map-type="route-comparison"` sobre **ficheros locales del almacén**, con perfil de elevación (ahora con **altitud en el cursor**, mejora compartida #56); debajo, un **espejo completo** de las métricas del metadato (resumen, cinemática, elevación con reparto por pendiente, curvas técnicas con racimos, puntuación), leído en servidor, con botones **«info»** accesibles (hover/foco/tap). Editor **botón→Modal** (fecha + `trip` informativo + tres cargas locales), **ciclo de vida de dos estados** (no validado / validado solo-lectura; «Borrar ficheros» única vuelta) y barra lateral heredada de `route-comparison` (sin URLs de GPX ni estadísticas manuales) bajo candado. Almacenamiento en `uploads/routes/recorded/<year>/<month>/` (`<md>.gpx` / `<md>_track.gpx` / `<md>_metadata.json`; sufijo `(N)` **antes de la extensión** en `assetSuffix`), vía endpoint REST `enterprise/v1/route-metadata` (POST validado por hash/duplicidad + `temp`+`rename`; DELETE para «Borrar ficheros»). **Puerta de publicación (§4.4):** bloquea **solo por completitud** del bloque, no por lo geográfico (`rest_pre_insert_post`→`WP_Error` + `lockPostSaving`); la **ingesta geográfica** (solo Tipo B/C) es **no bloqueante** y alimenta la taxonomía `regiones` por **unión/reemplazo** condicionada a `VALID`. Alta en #56 (v2.18.0). Ver §8 y §13.19. |
...
Los mapas **de tiles y coordenadas** usan **OpenLayers 9.2.4** (CDN: `cdn.jsdelivr.net/npm/ol@9.2.4/dist/ol.js`). El «Mapa interactivo de regiones» es un paradigma aparte, con **motor propio sobre SVG y sin OpenLayers** (ver su subsección al final de esta sección y §13.19).
...
### Mapa interactivo de regiones (SVG, motor propio — #43/#51/#54)

A diferencia del resto de mapas de esta sección, el bloque «Mapa interactivo de regiones» (`enterprise/interactive-region-map`) **no usa OpenLayers ni coordenadas geográficas reales**: es un **choropleth** sobre un **SVG maestro único georreferenciado** emitido inline, cuya unidad es la **región administrativa** (`<path>` con `id` = código ISO 3166-2 sin guion, más `data-admin`, `data-parent`, `data-name`). Es un paradigma **alternativo y complementario** a los mapas de tiles, no los sustituye ni reutiliza su motor. La navegación es continua entre niveles (`tier0` países / `tier1` regiones / `tier2` provincias) mediante animación del `viewBox` al *bounding box* de la unidad enfocada; los vecinos permanecen visibles y clicables. El motor es propio y **DOM-only**, `assets/js/region-map-frontend.js` (vanilla, sin *build*, sin `fetch`: toda la jerarquía y los nombres se leen de los `data-*` del propio SVG). El activo maestro y su modelo de árbol (un **par indivisible**) viven en el **almacén del sitio**, con copia semilla en `assets/maps/` (detalle en §13.19, #58). El color nace horneado en el activo (nivel 1) y el tema puede rehornearlo por CSS para el uso (nivel 2, configurable por el autor — #54), con el **fondo del lienzo como invariante no configurable**. **El mecanismo y las decisiones de arquitectura completas están en §13.19**; los ficheros de generación del activo (BE Map Studio, modelo de datos, estilo) se documentan en `interactive-maps/manual-enterprise-maps.md`.

**Conteo por región en el render (#57, Fase 3 de #45).** El `render.php` inyecta sobre cada `<path>` de región un atributo `data-count` con el número de **etapas (Tipo B/C) publicadas** que cruzan la unidad, tomado del `count` nativo del término `regiones` correspondiente (join `path.id` = `region_code`). Lo hace por **inyección aditiva** en el string del SVG (`preg_replace_callback`, sin tocar ningún otro atributo), con **tres estados**: región con término → `data-count="N"` (incluido `data-count="0"`); región cuyo `id` aún **no tiene término** (asincronía código↔geometría, §13.19) → **sin atributo**. El conteo se lee del core con `get_terms( hide_empty=false )`, se cachea en el transient `enterprise_regiones_counts` (TTL 12 h) y se **invalida** cuando cambia el conteo o el conjunto de términos de `regiones` (`edited_term_taxonomy`, `set_object_terms`, `created_term`, `delete_term`). #57 **solo deja el dato** en el DOM; **pintarlo** en el globo —o cualquier efecto por conteo— es **#48**, que lee `data-count` (`data-count="0"` = «0 etapas»; atributo ausente = «sin dato geográfico aún»). Alta en #57 (Fase 3 de #45; **sin bump**, consolida al cierre de #45).

**Reaparición del globo tras el zoom (#64, v2.18.1).** El globo con el nombre reaparece al terminar **cualquier** zoom de navegación (país, región, terminal, «volver», Europa), mostrando el nombre de la pieza que queda bajo el puntero, **sin exigir mover el ratón**. Antes de #64 una región **terminal** (un solo clic, sin sub-nivel) dejaba el cursor sobre la misma pieza tras el zoom y, al no re-dispararse `mouseover`, el globo no volvía; en táctil no aparecía en absoluto. En **táctil** el globo se muestra en el punto tocado y **persiste hasta el siguiente toque**. El mecanismo (reconciliación en el `onDone` de `animateVB`) y la decisión de la etiqueta determinista están en §13.19.

**Realce por conteo y adenda estética (#47, v2.21.0).** El bloque atenúa (50 %) las regiones sin etapas y «levanta» al hover las que tienen; llena la columna de contenido; `render.php` retira el prólogo XML del SVG (activo inviolable, D-1). Adenda: etiqueta del área enfocada y monograma de marca decorativo. Detalle y decisiones en §13.19.

### Mapa de ruta con metadatos (route-metadata-map — #56, v2.18.0)

El bloque **«Mapa de ruta con metadatos»** (`enterprise/route-metadata-map`) es una **alternativa a `route-comparison`** que, además de pintar el mapa, **carga la capa de metadatos** de la ruta y la usa para dos fines **independientes**: las **métricas** del GPX y el **inventario geográfico** (la asociación entrada↔región de la capa de datos de #45, §13.19). No sustituye a `route-comparison`. Ficheros: `blocks/route-metadata-map/render.php`, `assets/js/block-route-metadata-map.js` (editor), `assets/js/route-metadata-map-front.js` (front), `assets/css/route-metadata-map.css`.

**Editor.** Patrón **botón→`Modal`** (referencia `routes-by-location`, más simple). El modal captura la fecha (`year`/`month`/`day`), un `trip` **informativo** (etiqueta; no entra en el path ni en el nombre de fichero y **no se sanea**) y **tres cargas locales**: GPX planificada (opcional), GPX registrada (obligatoria) y JSON de metadatos (obligatorio). **Ciclo de vida de dos estados:** *no validado* (campos editables; «Guardar» valida y sube) / *validado* (datos en **solo lectura**; «Borrar ficheros» es la única vía de vuelta a editable). El lienzo muestra **placeholder** (no configurado) o **vista previa** (validado: el espejo de estadísticas). La **barra lateral** hereda la de `route-comparison` **menos** las URLs de GPX y las estadísticas manuales (sobreviven «Mostrar perfil de elevación» y «Mostrar estadísticas»), y añade el interruptor **«Usar los metadatos geográficos para el inventario del mapa»** (`useGeoInventory`); toda la barra queda **deshabilitada mientras no haya contenido validado**.

**Almacenamiento (endpoint REST `enterprise/v1/route-metadata`).** «Guardar» sube los ficheros a `POST enterprise/v1/route-metadata`, que **valida antes de escribir** (fecha real; consistencia por `gpx-data-sha256` entre el GPX registrado y su JSON; duplicidad) y guarda por **`temp`+`rename`** (todos o ninguno) bajo `uploads/routes/recorded/<year>/<month>/`, con nombres `<md>.gpx` (planificada, sin sufijo), `<md>_track.gpx` (registrada) y `<md>_metadata.json`, donde `<md>` = mes+día a dos dígitos. Ante **duplicidad con hash distinto** se añade un sufijo `(N)` **antes de la extensión** a los tres ficheros a la vez; el bloque persiste ese sufijo en el atributo **`assetSuffix`** (la base `<md>` se deriva de la fecha), y el front localiza sus ficheros con él. Con **hash igual**, el track ya se subió desde otro bloque: no se reescribe (aviso). «Borrar ficheros» elimina los ficheros del servidor vía **`DELETE`** del mismo endpoint.

**Front.** `render.php` solo pinta si el bloque está `validated`. Emite el mismo contenedor `.ent-map` con `data-map-type="route-comparison"` y las `data-gpx-url*` apuntando a las **URLs públicas** de los ficheros locales, de modo que **reutiliza el motor `map-frontend.js` sin tocarlo** (mapa OpenLayers de las dos rutas + perfil de la registrada). El **título del perfil se omite** (la altitud lo hace redundante). Debajo, el **espejo completo** de las métricas: lee el JSON **en servidor** y emite una tabla agrupada (cabecera + resumen/dificultad + cinemática + elevación con reparto por pendiente + curvas técnicas con racimos + puntuación), con formato numérico español y **filas omitidas si el dato falta**. Cada etiqueta puede llevar un botón **«info»** —solo si tiene entrada en el mapa localizado `enterprise_route_metadata_stat_info()`— cuyo tooltip es **accesible y táctil** (`<button>` + `aria-describedby` + `role="tooltip"`, activado por `route-metadata-map-front.js` en hover, foco y tap). El espejo usa tokens de color **funcionales** nuevos de `:root` (`--slope-1/2/3`, `--red-tint`/`--red-dk`; documentación de marca pendiente en #61).

**Mejora compartida — altitud en el perfil.** `map-frontend.js` muestra ahora la **altitud «N m»** del punto bajo el cursor del perfil de elevación (el dato ya viajaba en el track). Al ser el motor compartido, la mejora **también aparece en `route-comparison`, `route-map` y `animated-route-map`**.

**Puerta de publicación e ingesta (§4.4).** El fichero de metadatos tiene **dos propósitos independientes** —métricas (no geográficas) e inventario geográfico—, y **lo geográfico NO bloquea la publicación**. La **única puerta dura** es la **completitud** del bloque: al publicar, `enterprise_rmm_publish_gate` (filtro `rest_pre_insert_post`) veta con `WP_Error` si algún bloque `route-metadata-map` no está `validated` (mensaje al editor; complementado en cliente por `lockPostSaving`); **no se condiciona al tipo de entrada**. La **ingesta geográfica** (`enterprise_rmm_ingest_geo`, en `rest_after_insert_post`, **solo Tipo B/C `etapa`**) es **no bloqueante**: recompone los términos `regiones` del post como **unión de los bloques que contribuyen** —`validated` + `useGeoInventory` activo + metadato `validation_status == "VALID"`— mapeando `administrative_regions[].id → region_code → término`, y los fija por **reemplazo** (`wp_set_object_terms`, todos los niveles) → WordPress recalcula el `count`. La unión vacía (sin bloques, o todos apagados/no válidos) **retira** los términos previos, de modo que borrar un bloque o apagar el interruptor **decrementa** el conteo. `VALID` garantiza aguas arriba que las regiones están en el árbol, por lo que **no hay comprobación separada de existencia de término**. (Diferido a #60: persistir `useGeoInventory=off` + aviso en el editor cuando el metadato no es válido; la ingesta ya re-chequea `VALID` en servidor, así que el conteo es correcto sin ello.)

**Traza de cierre.** Implementado por el Desarrollador en `bffe645` (esqueleto), `e4e064d` (editor), `5bc87d5` (subida/validación), `aaa1658` (front), `1d82214` (altitud), `4eb3cc6` (tokens del espejo), `dd13a66`+`ed530d3` (ingesta+puerta), validado por Juanjo en WordPress real. Requisitos: `claude/requirements/closed/requirements-56-bloque-route-metadata-map.md` (no versionado). **bump 2.17.0 → 2.18.0** (adelanta el bump que estaba previsto para la Fase 5 de #45).
...
### 13.19 Bloque «Mapa interactivo de regiones» (#41): mecanismo y arquitectura
...
**Modelo vigente — SVG maestro único georreferenciado.** Un **único SVG maestro georreferenciado** con capas por nivel administrativo (`tier0`/`tier1`/`tier2`), emitido **inline** en el DOM dentro de `<div class="ent-region-map">`. Cada `<path>` porta su propia identidad y jerarquía en atributos `data-*`: `id` (código ISO 3166-2 sin guion), `data-admin` (nivel administrativo), `data-parent` (unidad contenedora) y `data-name` (nombre de visualización). El SVG maestro y **el árbol** que lo acompaña (JSON) forman un **par indivisible** (su origen y almacenamiento, más abajo: #58). El `render.php` emite el maestro inline (el SVG se emite tal cual, **sin `wp_kses`** —eliminaría elementos/atributos SVG— y sin `<script>` inline: la interactividad necesita los nodos `<path>`; desde **#57** el `render.php` realiza además una **inyección aditiva controlada** de `data-count` por región —no saneado ni despojo: el resto del SVG se emite byte a byte igual—, coherente con este invariante); el CSS oculta `tier1`/`tier2` en el render estático; **encolado condicional propio** (`has_block`+`filemtime`), **separado** del de OpenLayers, que **no se carga**. Elegido frente al modelo *swap* anterior (un SVG por nivel y por país) porque un maestro único georreferenciado da **navegación continua**: al enfocar una unidad, los vecinos **no desaparecen**.

**Origen del activo — par mapa+árbol en el almacén del sitio (#58).** El par **indivisible** mapa+árbol (`enterprise-eu.svg` + `map-regions-global.json`), generado junto aguas arriba, **deja de ser activo de git y pasa a vivir en el almacén del sitio** (`uploads/enterprise-maps/`); las copias del tema en `assets/maps/` quedan como **semilla/respaldo** de una instalación nueva. Se **suben juntos** desde una página de administración propia **«Enterprise Moto»** (menú de nivel superior; en #58 **solo administradores** —`manage_options`—, #55 la amplía a editores al añadir «Sincronizar»), con validación **transaccional**: los dos presentes, el árbol con forma esperada (`_meta` + `tree` no vacío) y —bloqueante— el sello **`data-tree-sha256`** que el generador escribe en el `<svg>` raíz **casa** (`hash_equals`) con el `sha256` de los bytes exactos del árbol subido; si algo falla, no se guarda nada (escritura por temporales + `rename`: los dos o ninguno). El sello prueba **«misma generación»**, no paridad nodo-a-nodo (respeta el async código↔geometría, más abajo). Un **resolutor único** `enterprise_map_asset_path($filename)` (almacén→tema) sirve el fichero; `render.php` lo usa para el SVG (l. 22) y #55 lo reutilizará para el árbol. Coherente con «el contenido del mapa es contenido, no diseño»: cambiarlo es operación de contenido, **sin subir versión**. (Materializa la Decisión B/Opción 2 de #45; árbol **inmutable**, subido verbatim. Implementado en `5dca2a9` + `c632b53`, validado en WordPress real; el **bump va consolidado con el cierre de #45**.)

**Capa de datos — taxonomía `regiones` + «Sincronizar» (#55).** La relación entre las entradas y las unidades del mapa se materializa como una **taxonomía propia plana `regiones`** sobre `post` (no jerárquica; registrada **mínima en la parte pública** —`public=false`, `publicly_queryable=false`, `rewrite=false`, `show_ui=true`—: sin páginas ni URLs por región, porque el **destino del clic es de #46**). Cada término es una unidad del mapa y porta dos term meta: **`region_code`** (código ISO = `id` del `<path>` del maestro = **clave de join**; el slug, `strtolower(code)`, no es portante — los nombres colisionan, el código no) y **`region_admin`** (nivel administrativo). Los términos se **siembran y actualizan** desde el **árbol autoritativo** (`map-regions-global.json`, leído del almacén del sitio vía el resolutor `enterprise_map_asset_path()` de #58) con la herramienta **«Sincronizar regiones»**, alojada en la página de admin «Enterprise Moto», que #55 **amplía a administradores + editores** (la subida del par de #58 sigue **solo administradores** dentro de la misma página). La sincronización es **idempotente y no destructiva** (Decisiones A–F de #45): recorre el árbol **completo** (todos los niveles, incluidos los nodos sin geometría —el async código↔geometría es esperable y no bloquea, ver más abajo—), **empareja por `region_code`**, y clasifica en cuatro grupos —**nuevas** / **nombres a actualizar** / **descolgadas con entradas** / **descolgadas sin entradas**—; **nunca borra términos ni reasigna posts**: un término con posts queda **protegido** aunque su código desaparezca del árbol, y «actualizar» refresca **nombre y nivel** (`region_admin`), nunca la identidad ni el código (Decisión F + Opción A). El flujo es **vista-previa-y-confirmar siempre** (Decisión C): «Analizar» produce el reporte como *dry-run* (sin escrituras) y **solo desde ese reporte** se alcanza «Aplicar estos cambios» —doble candado: el botón de aplicar vive dentro del panel del reporte y el *handler* exige que exista el análisis previo (transient del usuario), rechazando cualquier `apply` que no venga de una previa—; «Aplicar» recalcula sobre el árbol vigente (idempotente). La **migración** de posts entre términos ante un cambio de código ISO es una acción deliberada aparte (**#59**). Esta capa solo **construye el catálogo**: el **conteo** por región y la **inyección de `data-count`** en el render son fases posteriores (**#56** asociación/ingesta/puerta, **#57** conteo). Implementado en `aea36f9` + `0ec9fde` + `ccac51e` + `e743f3d` (este último la corrección de la Decisión C), validado en WordPress real; **sin bump** (consolidado con el cierre de #45). *(Esta capa taxonómica **supera** el modelo original que describía las keys como un **metadato JSON** que el tema solo consumía: el **almacén es la taxonomía** y el JSON de metadatos queda como **entrada de ingesta**. Reconciliado en el párrafo «Modelo región↔contenido» de más abajo, al cerrar la **Fase 5** de #45.)*

**Motor — DOM-only (Decisión H).** `region-map-frontend.js`, vanilla JS **sin *build*** y **DOM-only**: sin `fetch`; toda la jerarquía, los nombres y la navegabilidad se leen del propio SVG (`data-*`), y los hijos de una unidad son `path[data-parent=<id>]`. Mecanismos:

- **Zoom** = animación del `viewBox` al *bounding box* del nodo enfocado (`getBBox` + `requestAnimationFrame`).
- **Visibilidad** por clases: `ent-active` (enfocado), `ent-dimmed` (atenuado), `ent-hidden` (oculto).
- **Vecinos**: atenuado = **visible + clicable** en todos los niveles; el clic salta directamente a la unidad (por `data-admin`), incluido el salto **región↔región transfronterizo**.
- **Regiones terminales**: las unidades admin-1 sin sub-nivel se enfocan **sobre sí mismas** (enfocabilidad según el nivel administrativo); la provincia es la hoja que redirige (#46).
- **Globo** bilingüe; **«volver»** = alejar un nivel; interacción por **ratón y toque**.

**Globo — reaparición reconciliada tras el zoom (#64, v2.18.1).** El globo se enciende con el `mouseover` habitual, pero además se **reconcilia al terminar cada zoom** de enfoque: en el `onDone` de `animateVB` —por el que pasan **todas** las rutas de zoom (país, región, terminal, «volver», Europa)— el motor decide qué mostrar según el último punto del puntero/toque (`state.ptrX/ptrY`, actualizado en `mousemove` y `click`). Esto arregla las **terminales** (un clic, sin sub-nivel): tras el zoom el cursor queda sobre la **misma** pieza, no cruza a ninguna nueva y `mouseover` no se re-dispara, así que sin la reconciliación el globo no volvía (y en táctil no aparecía, porque el navegador solo finge `mouseover` al tocar una pieza distinta de la última). En **táctil** el globo se muestra en el punto tocado y **persiste hasta el siguiente toque**: un toque que enfoca lo apaga (al arrancar el nuevo zoom) y lo reenciende (al terminarlo) para la nueva pieza; un toque a mar/vacío/pieza oculta lo apaga; la hoja provincia (clic no-op → #46) lo mantiene.

**Decisión de arquitectura (#64) — etiqueta determinista en el enfoque a pieza única; hit-test solo sin ella.** *Contexto:* el diseño cerrado de `requirements-64` (§3, decisiones 1–3) fijaba que la reconciliación mirara **siempre** la pieza bajo el puntero por hit-test (`document.elementFromPoint(ptrX,ptrY)` → `closest('path')`). *Problema (validación táctil, capturas de Juanjo):* tras un zoom de **enfoque a una pieza única**, ese píxel es un dato **obsoleto** del momento del toque; con el nuevo encuadre suele caer sobre el **vecino atenuado** que asoma en el margen, mostrando el nombre equivocado (Navarra mostraba «Nueva Aquitania»; la Comunidad de Madrid, «Castilla-La Mancha»). *Decisión (operador):* en un zoom que enfoca **una pieza determinada** —la rama terminal de `focusRegion`/`focusCountry`, donde el motor ya la tiene «en la mano»— el nombre es **determinista** y se muestra el de **esa** pieza (`state.pendingBalloonPath`), sin hit-test; el hit-test por `elementFromPoint` queda **solo** para los zooms **sin pieza única**: «volver», Europa y los *drills* que destapan una sub-capa (país→regiones, región→provincias). *Consecuencia:* el globo nunca vuelve a mostrar el nombre de un vecino tras enfocar una unidad. Commit único `dc562ca`.

**Separación de entrada ratón/táctil (#66.1).** El motor deja de escuchar eventos de ratón y pasa a **Pointer Events** con bifurcación por `pointerType` (D1=A): una sola familia de listeners en el contenedor (`pointerover`/`pointermove`/`pointerout`/`pointerup`) que ramifica ratón/lápiz (`'mouse'`/`'pen'`) y táctil (`'touch'`). Cambio **sin comportamiento visible**: el ratón hace lo de siempre; el táctil —que antes emergía de los eventos de ratón sintéticos del navegador— se reproduce ahora en su propia rama (globo al tocar; ignora `pointermove`/`pointerout` para conservar «persiste hasta el siguiente toque», #64). La activación (drill) comparte hoy un despacho único (`activate`), separado por rama para que 66.2 (ratón) y 66.3 (táctil) diverjan sin reintroducir un camino común. El globo y el botón «volver» conservan su `click` propio. Los eventos sintéticos del navegador se siguen emitiendo, pero el contenedor **ya no los escucha** (sin doble disparo); su **supresión** y el **rediseño del gesto táctil** son #66.3. Habilitador de 66.2/66.3. Implementado por el Desarrollador en `352295e` (familia hover) + `8885d73` (activación), validado por Juanjo en WordPress real; **sin subir versión** (sub-fase del plan #66, cambio no visible). Requisitos: `claude/requirements/closed/requirements-66-1-separar-raton-tactil.md`.

**Navegación con ratón del último tier (#66.2).** Con el **ratón**, al entrar en una pieza terminal **con rutas** (`!hasChildren` + `data-count>0` + Página-destino configurada) el globo **sigue al cursor** (deja de anclarse) a la misma distancia corta que en los tiers superiores, y muestra **nombre + «Rutas: N» + la pista de acción «Ver las rutas»**; el **clic en cualquier punto de la región** navega a su página (`buildRegionHref`), y el globo pasa a ser **etiqueta no clicable** (`pointer-events:none`). Las regiones sin rutas (`0`/ausente) no muestran pista y el clic conserva su significado (drill o no-op). El **táctil no cambia**: conserva su globo **anclado y clicable** y la navegación por el clic del globo, hasta su rediseño en #66.3. El cambio se acota al input con un 5º argumento `isMouse` de `showBalloon`. **Mecanismo del reconcile (corrección de spec, IA):** `reconcileBalloonAtPointer` corre en el `onDone` del zoom, **aguas abajo** de la bifurcación por `pointerType`, y lo ejecutan **ambos** inputs; un `isMouse` fijo (como decía el spec) habría dado comportamiento de ratón al táctil, rompiendo el invariante mouse-only. Se resuelve con `state.lastInputMouse`, que fijan los handlers (`pointerover`/`pointerup`) y lee el reconcile — sin ese rastreo, la separación de #66.1 no alcanzaba al reconcile. **Estilo** `.is-link` + pista por tokens (§13.18), reutilizando el acento de marca `--gold` (CTA); **sin token nuevo** (a diferencia de `--balloon-fg` de #48), sin follow-up de manual de marca. Implementado por el Desarrollador en `41d25a6` (globo sigue + pista) + `762be2e` (clic navega) + `2b71998` (estilo), validado por Juanjo en WordPress real; **bump 2.22.0 → 2.22.1** (consolida el diferido de #66.1). Requisitos: `claude/requirements/closed/requirements-66-2-navegacion-raton.md`.

**Rediseño de la navegación táctil (#66.3, v2.22.2).** Cierra el plan #66. La pantalla táctil **no tiene *hover***, así que la navegación táctil pasa a un **modelo de dos toques** (uniforme por nivel): el **1.er toque revela** —enciende el globo (sustituto del hover) y **acentúa** la pieza; **persiste hasta el siguiente toque**— y **arma** la pieza (`state.armedPath`/`confirmPending`); el **2.º toque** sobre esa misma pieza **confirma según el TIPO de pieza, no según el nivel**: **drillable → entra** (zoom al `tier` siguiente); **terminal con rutas → navega** a su página (#46); **terminal sin rutas → no-op** (sigue resaltada y con globo). Un toque en otra pieza re-arma; en mar/vacío cierra; `pointercancel` **aborta** (limpia el armado para que no herede un falso «segundo toque»). El **salto a vecino** (pieza atenuada de fuera del foco, clicable) se comporta como su tipo de pieza (país en `Nivel-1`, región en `Nivel-2`). **Acento por clase** `.ent-touch-accent` (espejo de las reglas de acento `:hover`), que el motor pone/quita **solo** en `pointerType==='touch'`, reutilizando `--ent-hover-accent` (sin literal). **Globo anclado en el punto tocado** (sobre la geometría de la pieza, no en su centro) y, en táctil, **`pointer-events:none`**: navega el **2.º toque sobre la pieza**, no el globo. **`touch-action: manipulation`** en `.ent-region-map` desactiva el zoom por doble-toque del navegador, que secuestraría el gesto de dos toques. El **reconcile de #64 queda suprimido en táctil** (tras cualquier zoom —drill o «volver»— no reaparece globo ni queda pieza acentuada; el globo solo aparece por un 1.er toque); **la rama de ratón (#66.2) queda intacta** (el reconcile sigue leyendo `state.lastInputMouse`). El botón «volver» conserva su `click` (sube un nivel; tras él, sin acentos ni globo). Implementado por el Desarrollador en `f33c5a6` (acento por clase + acota `:hover`), `7664261` (gesto revela/confirma) y `8ba4578` (`touch-action` + globo etiqueta), validado por Juanjo en WordPress real (incl. iPhone); **bump 2.22.1 → 2.22.2**. Requisitos: `claude/requirements/closed/requirements-66-3-navegacion-tactil.md` (no versionado). La **tabla completa del gesto** está al final de esta subsección y, en versión de usuario, en `interactive-maps/manual-enterprise-maps.md` («Estructura del mapa: zoom tiers») — **misma tabla, mantener en sync**.

**Desviación autorizada (#66.3) — el acento `:hover` acotado a `@media (hover: hover)`.** *Contexto:* `requirements-66-3` (§1.4) pedía **no tocar** las reglas `:hover` del acento (ruta de ratón). *Problema (validación en el iPhone de Juanjo):* en iOS el `:hover` se queda **pegado** tras el toque y la pieza quedaba **doble-acentuada** —resaltada a la vez por `:hover` y por `.ent-touch-accent`—. *Decisión (operador, ante el bug):* acotar las **cuatro reglas de acento `:hover`** de pieza a **`@media (hover: hover)`**, de modo que solo existan en dispositivos con hover real; en táctil el acento lo da **en exclusiva** la clase. El ratón no cambia (cumple `hover: hover`). No introduce token ni requisito nuevo; es un **fix de compatibilidad táctil** (`f33c5a6`).

**Terminología — `tierN` (capas del SVG) vs `Nivel-N` (estados de navegación).** Dos ejes que no hay que confundir (fijado en #66.3). **`tierN`** = **capa del SVG maestro** (`<g id="tierN">`, con `tierN = admin-N`: tier0 países, tier1 regiones, tier2 provincias); está horneada en el activo **inviolable** (D-1) y de ella cuelgan el CSS (`[id="tierN"]`) y el motor (`#tierN path`), así que **no se renumera**. **`Nivel-N`** = **estado de navegación/zoom** del usuario: `Nivel-0` continente (sin ISO), `Nivel-1` país enfocado (admin-0), `Nivel-2` región enfocada (admin-1), `Nivel-3` provincia (admin-2); regla `Nivel(N+1) ≈ admin-N`. La capa es *dónde se dibuja* una pieza; el nivel es *el estado* en que está el usuario. (Detalle didáctico en el manual, «Estructura del mapa: zoom tiers».)

**`assets/maps/` es *fallback*, no referencia de contenido.** Las copias del par mapa+árbol en `assets/maps/` son **semilla/respaldo** de una instalación nueva (§13.19, #58): el activo operativo vive en el almacén del sitio (`uploads/enterprise-maps/`) y el `data-count` se **inyecta en tiempo de render** (`render.php`, #57, sobre **cualquier** `<path id>` presente en `enterprise_regiones_counts()`, **incluidos códigos de país** —p. ej. España muestra «Rutas: N»—). Por tanto, **el SVG semilla de `assets/maps/` NUNCA es referencia para razonar sobre el contenido navegable con datos poblados** (qué unidades tienen `data-count`, si un país tiene conteo, etc.): para eso mandan la **función real** y el **runtime**.

**Tabla del gesto táctil (#66.3)** — misma que el manual («Estructura del mapa: zoom tiers»); mantener en sync.
...
**Motor *content-agnostic* (principio, #52).** El mapa como artefacto es un **asset**; **qué unidades contiene un maestro dado es contenido, no arquitectura**, y por tanto no se documenta aquí. El motor **no lleva ningún código específico de contenido**: navegabilidad, jerarquía y nombres son **dirigidos por el dato** del propio SVG, de modo que **sustituir el maestro por uno más amplio no exige tocar código**. (Principio consolidado al retirar #52 el último acoplamiento motor↔contenido que quedaba; su cierre formal va conjunto con #53.)

**Modelo región↔contenido (definición → materializado por #45).** La unidad de asociación es el **track (GPX)** de una etapa (Tipo B/C, §3); un track **pertenece a las unidades por las que pasa** → un **conjunto de keys por nivel**. Las keys se **cocinan con herramientas externas** al tema, con la **misma codificación** que los `id` de los `<path>` del maestro (contrato clave). **Almacén (reconciliado por #45):** el modelo original describía esas keys como un **metadato JSON** de la etapa que el tema «solo consume»; con #45 el **almacén es la taxonomía `regiones`** (arriba, «Capa de datos… #55»), y el **JSON de metadatos** —cargado por el bloque «Mapa de ruta con metadatos» (#56)— pasa a ser la **entrada de ingesta**, no el almacén: al publicar una etapa Tipo B/C, la puerta de #56 lee ese JSON y, si es `VALID`, **asigna los términos** de `regiones` (todos los niveles) que WordPress cuenta, y #57 inyecta ese conteo como `data-count` en el render. El tema **ya no «solo consume» un JSON**: lo **ingiere** hacia su propio almacén taxonómico consultable. El **cómo/dónde** se generan las keys aguas arriba sigue **fuera del alcance de este plan**.

**Modelo de consumo aguas abajo — destino del clic (A+B, #46; Fase 4 de #45).** Decidido por el operador (2026-08-02) el destino de la unidad terminal al hacer clic: una **página de destino por región** realizada con una **plantilla dedicada** que recibe el **`region_code`** de la unidad **desde la URL** (el enlace del globo lo transporta), no el archivo nativo del término —la taxonomía `regiones` se registró **sin parte pública** (`public=false`), así que no hay archivo de término que sirva de destino—. La página muestra **dos carruseles horizontales**: (1) **etapas** (Tipo B/C) que cruzaron la región —las entradas del término `regiones` de la unidad terminal, **misma fuente** que el `count`/`data-count` de #57, de modo que el número del globo **coincide exacto** con el carrusel—; y (2) **entradas Tipo D** que **contienen** alguna de esas etapas (por el filtro de viaje `_post_viaje_*`), como lista aparte con su propia cuenta —**nunca** páginas «Colección de viajes» (corrección, ver más abajo)—. El modelo **A+B disuelve el descuadre de contador** que habrían producido tanto la variante «solo viajes» (un viaje agrupa varias etapas → menos ítems que el conteo) como la de «etapas agrupadas por viaje». **Frontera #45↔#46:** #45 entrega, dado el `region_code` de la unidad terminal, **el conjunto de sus etapas publicadas (+ conteo)** —el mismo origen que la inyección de `data-count` de #57—; la **derivación de los viajes** (carrusel 2) y el **montaje de la página** son trabajo de **#46**. *Verificado en el código (HEAD `7d28584`):* el vínculo **etapa→viaje es indirecto** —un viaje no guarda enlace a sus etapas; se define por bloques de filtrado que resuelven a un conjunto de IDs (`enterprise_collection_post_ids`, vía `enterprise_stage_query`)—, y el recorrido **inverso** (de una etapa a los viajes que la incluyen) **ya se realiza** al guardar una entrada (`enterprise_recache_collections_on_post_save`), de volumen bajo y cacheable; el carrusel 2 es **factible sin datos nuevos** (no es padre-hijo ni meta directo — «no tomes el todo por la parte»). Ambos carruseles reutilizan la card `.trip-card` (`enterprise_trip_card_data`, que ya distingue etapa/viaje por *badge*) dentro del carrusel horizontal compartido (`enterprise_carousel_assets`, `carousel.css`/`carousel.js`). El **enlace del globo** hacia la página es **análogo —no reutilización—** al «→ Entradas relacionadas» de `routes-by-location` (`assets/js/map-frontend.js`): aquel motor es OpenLayers; este es el SVG DOM-only. La navegación arrastra un **nuevo contexto de origen `from_region`** integrado en `enterprise_nav_origin_params()` y en el arrastre de ancestro (§6/§13.1, §13.11/§13.13), **por definir en #46** (hoy `from_region` aún no existe en el código). **Prerrequisito de UX ya resuelto:** el globo que porta el enlace es el de hover, cuya reaparición en las terminales tras el zoom arregló **#64**. Con esto, el **«punto de diseño mayor» de #46** —la agregación etapa→viaje (Tipo D)/listados— queda **resuelto** como **entradas Tipo D que contienen la etapa** (por el filtro de viaje). *(**Corrección 2026-08** —operador + Desarrollador, verificado en código—: el modelo previo «pertenencia a colecciones «Colección de viajes»» / «A+B» queda **anulado y sustituido**. Motivo: `regiones` se asigna **solo a Tipo B/C** (`enterprise_rmm_ingest_geo`, `functions.php`: `if ( 'etapa' !== $tipo ) return;`) y las «Colección de viajes» agrupan Tipo D, siendo la única existente, «Salidas de un día», de **Tipo C**, cuya única etapa ya sale en el carrusel de etapas → habría duplicado/descuadre. El carrusel de viajes son **entradas Tipo D** cuyo filtro `_post_viaje_cat_ids`/`_post_viaje_tag_ids`/`_post_viaje_tag_rel` contiene la etapa, resuelto como en `enterprise_calculate_viaje_stats`; el mecanismo se especifica en `requirements-46` y se consolidó al cerrar #46 (v2.20.0); el modelo construido se documenta a continuación.)*

**Página de destino por región — modelo construido (#46, v2.20.0).** El clic en la unidad terminal lleva a una **página de destino por región** con plantilla dedicada `page-templates/template-region-destino.php`, resuelta por `?region=<code>` → término `regiones` por `region_code` (**Opción A**; la taxonomía sigue `public=false`; la **Decisión D se reabrió y se confirmó**, sin cambio de registro). La página se elige en el Customizer (`enterprise_region_dest_page`, `dropdown-pages`) y su URL la construye `enterprise_region_destination_url()`. Muestra **dos carruseles**: (1) **etapas** del término —fuente única `enterprise_region_stage_query($term_id)`, la MISMA que alimenta el prev/next del contexto `from_region`—; y (2) **entradas Tipo D** que las contienen —`enterprise_region_trip_ids()` sobre `enterprise_viaje_stage_ids()`—, **nunca** páginas «Colección de viajes» (modelo previo anulado, ver corrección arriba). La derivación viaje→etapas es el **filtro `_post_viaje_*`** del viaje, extraído a `enterprise_viaje_stage_ids()` (fuente única, cambio *behavior-preserving* sobre `enterprise_calculate_viaje_stats`); el mapa región→viajes se cachea en un único transient `enterprise_region_trips_map` (reconstrucción perezosa + TTL 12 h) invalidado en `enterprise_post_stage_save` (cubre guardar etapa Tipo B/C y viaje Tipo D).

**Hero (#46).** Nombre de la región + nº de etapas + nº de viajes; subtítulo «Rutas que cruzaron esta región»; ticker del campo **«Nombre en el ticker»** (`_post_ticker_name`, cae al título si vacío); el nombre bilingüe «Nativo [Español]» muestra el español; fallback «¡Te pasaste de frenada!…» cuando el código no resuelve.

**Contrato de navegación `from_region` (#46, §6/§13.13).** El destino es un listado de origen: cada card de **etapa** estampa `from_region` (id de la página de región) + `region_code` + `region_src`. `single.php` reconstruye el prev/next re-ejecutando `enterprise_region_stage_query` sobre el término (**no** cat/tag —diferencia con `from_loc`—), «Volver» regresa a la página de región (`?region=`+`region_src`) y «← Volver al mapa» se arrastra por `region_src` (gemelo de `loc_src`). El **carrusel 2 (Tipo D) también estampa `from_region`** (#65, reabre y sustituye la §3.5 de #46, que lo dejaba en salto plano): al abrir un viaje desde el carrusel 2, `single.php` **bifurca el prev/next por tipo de entrada** (`_post_tipo`) —un **viaje** recorre los **viajes publicados** de la región (`enterprise_region_trip_ids`, filtrado a `publish`, mismo origen y orden que el carrusel 2), una **etapa** recorre las etapas del término—, con etiquetas «Viaje anterior / Siguiente viaje» para el viaje. El **«Volver» del contexto de región se unifica** en «← Volver a la región» para etapa y viaje. Al llevar el viaje contexto propio, sus etapas heredan `from_region` como ancestro y su «Volver» deja de depender del *referer* — lo que **elimina el bucle viaje↔etapa** (§13.20).

**Globo — enlace al destino (#46; el globo completo es #48).** El enlace vive en el **globo** de hover: `render.php` emite `data-region-dest` (permalink de la Página-destino + `region_src`) solo si la página está configurada y publicada; el motor lo lee (`regionDestBase`) y, en una pieza **terminal** (`!hasChildren`) con `data-count>0`, ancla el globo y lo hace clicable (`window.location.href`), navegando a la página de la región. El globo muestra además el **nº de entradas** (`data-count`) con la etiqueta «**Rutas: N**» y estilo por tokens (§13.18), entregados por **#48** (v2.22.0). El **anclaje + clic del globo** descritos arriba son, desde **#66.2** (v2.22.1), el camino **táctil**: con el **ratón**, el globo del último tier **sigue el cursor**, muestra «**Ver las rutas**» y **el clic en la región** navega (el globo es etiqueta no clicable). El **rediseño del gesto táctil** sigue en **#66.3**. Invariante uni-provincial: `count(admin-1)=count(admin-2)` por asignación de ancestros..

**Color del mapa — dos etapas (nivel 1 → nivel 2).** El **color estructural** del mapa —el **fondo** (`canvas`, un `<rect>` a pantalla completa), el **relleno base** de cada `<path>` y el **estilo por nivel de zoom** (`tier0/1/2`, que puede redefinir cualquier propiedad, incluido el relleno)— **no es color CSS del tema**, así que el contrato de tokens de §13.18 **no lo alcanza**. Nace **horneado en el activo** por la herramienta generadora (**nivel 1**): a partir del estilo editorial del tema, que viaja en el **JSON de estilo**, el generador lo escribe como **atributos de presentación** del SVG (ver el manual de mapas, «Colores del mapa» y «Asset: estilo del mapa»). Cuando el mapa **se incorpora como activo del tema** (**nivel 2**), el tema es su dueño y puede **rehornearlo/adaptarlo según el uso**; el mecanismo concreto (reprocesar el SVG, tratarlo por CSS, o ambos) es diseño de nivel 2 y se fija en su fase. Principio: **generar alineado (nivel 1) → incorporar y rehornear para el uso (nivel 2)**.

**Nivel 2 — colores configurables por el autor (#54).** El mecanismo de nivel 2 que se dejó abierto arriba se fija como **CSS + custom properties**. El bloque expone un interruptor **«Colores del mapa»** con dos modos. En **«Los del propio mapa»** (`asset`) no se emite ningún override y manda el activo (nivel 1). En **«Personalizar»** (`theme`), `render.php` emite en `.ent-region-map` las clases de estado (`is-color-theme`, `palette-*`) y **custom properties inline** de color (saneadas con `sanitize_hex_color`), grosor y opacidad por nivel; el CSS las consume con `var(--…, <valor horneado>)`, usando **el valor del activo como fallback**, de modo que el modo `asset` es idéntico al de antes de #54. La paleta es un **juego cerrado** (Editorial / Enterprise, en desplegable, extensible): el editor limita los colores a los de la paleta activa; `stroke-width` y `opacity` son numéricos y no están sujetos a esa restricción. El **acento de hover** deja de ser el literal `#f2c118` y pasa a `var(--ent-hover-accent, #c9a010)`, con `#c9a010` (`--gold-dk`) por defecto también en modo `asset`. El botón **«volver»** tiene dos presets sólidos según el tipo de lienzo que el autor declara (`back-light`/`back-dark`), con criterio de **contraste** con el lienzo (claro → pastilla oscura, oscuro → pastilla clara); el lienzo sigue siendo nivel 1 y no se re-hornea. **Selectores elegidos para no romper el motor ya validado:** el tinte base va en `.ent-region-map > svg path` y los niveles en `[id="tierN"] path` (sin especificidad de `id`) y **antes** del bloque de interacción, de modo que el resalte de hover y el atenuado de vecino (`.ent-dimmed`, por clase) sigan ganando. **Consecuencia fiel al modelo `defaults + override por nivel`:** con el activo actual de tres niveles, los controles de **nivel base** (borde/grosor/opacidad base) quedan sobrescritos por cada tier; solo «Relleno de las tierras» y los controles por nivel tienen efecto visible. Es el primer bloque con `attributes` (#43 lo registró sin ellos; la categoría sigue siendo #39).

**Corrección colateral (fix latente de #51).** La regla del fondo del lienzo usaba el selector descendiente `.ent-region-map svg`, que alcanzaba también el `<svg>` del icono del botón «volver» (lo estiraba a `width:100%` y le pintaba el fondo crema del lienzo por detrás de la flecha). Se acotó al **hijo directo** `.ent-region-map > svg`, que es solo el mapa (el icono cuelga del `<button>`). (#54)

**Invariante — el fondo (`canvas`) no diverge: lo declara el activo, lo aplica el tema.** El **fondo del lienzo** es la **única** entidad de color exenta de la libertad de rehorneado del nivel 2: tiene **una sola fuente de verdad**, el **canvas horneado en el activo** (nivel 1). El tema (nivel 2) **no guarda un valor de fondo propio e independiente**: lo **deriva del propio activo**, leyéndolo de él, de modo que **no puede divergir por construcción**. *Por qué* (verificado en runtime): el zoom anima el `viewBox` del SVG. El `<rect width="100%" height="100%">` del maestro tiene sus porcentajes referidos al `viewBox`, así que **deja de cubrir** en cuanto el `viewBox` se recorta al hacer drill; la capa que cubre el área renderizada bajo cualquier `viewBox` es el **`background-color` del elemento `<svg>`**, independiente del `viewBox`. Ese `background-color` **debe valer exactamente el canvas del activo**; si el tema lo fija por su cuenta y diverge, al hacer drill los huecos entre `<path>` se pintan con el color equivocado del tema en vez del canvas. *Mecanismo:* `render.php` extrae el color del primer `<rect>` del maestro y lo expone como custom property `--ent-region-canvas` en el contenedor `.ent-region-map`; el CSS la consume en el `<svg>` (`background-color: var(--ent-region-canvas, var(--white))`), que cubre el área renderizada bajo cualquier `viewBox`. Si no puede leerse, cae al *fallback* blanco (`var(--white)`). El `<rect>` declara el canvas (cubre en tier0) y ese `background-color` derivado lo cubre en el zoom — un único valor para los dos.

**UX del editor y globo.** El autor personaliza por región el **color** (por tokens, §13.18); navegación **automática** (sin URL manual) y sin estado «visitado». Globo de hover: **nombre + nº de entradas** («Rutas: N», estilo por tokens — #48, v2.22.0). Se descartan la «descripción» por región (anulada al redefinir #47) y la pista «acercar» (gesto evidente). La **pista de acción** («Ver las rutas» en terminales con `data-count>0`) se entrega para **ratón** en **#66.2** (v2.22.1), donde además el globo del último tier **sigue el cursor** y **el clic en la región** navega; la **afordancia táctil por nivel** y el **anclaje del globo** al tocar el último nivel quedan en **#66.3**.

**Ejecución — tres bloques, siete fases.** Cada fase con su sub-TODO y su documento de requisitos Arquitecto→Desarrollador:

- **Bloque I — Infraestructura de renderización:** #42 activos; #43 esqueleto del bloque `enterprise/interactive-region-map` + render estático; #44 navegación (hover, clic-zoom, táctil, volver). La **Fase I fue replanteada por #51** (modelo *swap* → maestro georreferenciado único).
- **Bloque II — Contrato de consumo de keys:** #45 (tras el Bloque I; depende del vocabulario de códigos de #42).
...
- **Derivados abiertos:** **#50** accesibilidad de teclado + lector de pantalla (retirada de #44; el motor es solo ratón/toque). **#49** quedó **cancelado** (el modelo del mapa ya trae los nombres localizados, coherente con el globo bilingüe).

**Riesgos registrados.** (1) estabilidad del **contrato de codificación** entre la herramienta externa y los `id` del maestro (si la fuente cambiara sus `id`, las keys dejarían de casar); (2) **agregación por tipo de contenido** — las keys viven en la etapa; cómo afloran los viajes (Tipo D) y los listados en el destino **era** el punto de diseño mayor de #46, **resuelto** como **entradas Tipo D** que contienen la etapa (por el filtro de viaje; ver «Modelo de consumo aguas abajo» y su corrección 2026-08 —el modelo previo de pertenencia a colecciones quedó anulado—); (3) **peso de admin2** → carga perezosa; (4) **accesibilidad** de *paths* irregulares; (5) coordinación de **#43 con #39** (categoría de bloques).

**Asincronía código↔geometría — nota para las fases que consumen el árbol (#45/#46/#48).** Que un nodo del árbol de regiones no tenga geometría en el maestro (un «huérfano») **no es un bug ni bloquea**: es el desajuste clásico de GIS entre estándares — el árbol se puebla de ISO 3166-2 (vía `pycountry`, al día), mientras la geometría de origen va **por detrás** y tarda en reflejar reorganizaciones administrativas. El **motor es *content-agnostic*** (ver arriba): no lee el árbol, solo el SVG, así que un desfase árbol↔geometría **no afecta a la navegación**. La coherencia de códigos se garantiza **aguas arriba, en la generación** del maestro (normalización + diccionario de alias; ver el manual de mapas), no en el consumo. Por tanto, al construir #45 (contrato de keys), #46 (redirect de nivel-3) y #48 (globo con conteos), un código presente en el árbol pero sin geometría —o al revés— es **esperable** y se trata como caso normal, no como un error que detenga la fase.

**Precisión para no diagnosticar en falso (el error a no repetir).** Que un `code` del árbol no figure como `id` en el SVG **no equivale** a que le falte geometría. Puede estar representado bajo su código canónico —colapso por alias de una unidad uniprovincial: Navarra `ESNA`→`ESNC`, que se dibuja como `ESNC` en `tier1` y en `tier2`— o cubierto por su ascendiente vía profundidad variable (una hoja admin-3 como `FR67`/`FR68` la cubre su padre en `tier2`). Un huérfano **real** es un nodo cuyo territorio no lo dibuja **nada** una vez aplicados el alias (`NATURAL_EARTH_ALIASES`, ver el manual de mapas) y el relleno de tiers. Contarlos por la resta de conjuntos `codes − ids` del SVG **sobre-cuenta**: marca también los colapsados por alias y los que caen por debajo de la profundidad del mapa (`tier2`), que no son huecos. La comprobación válida es territorial (¿queda la zona sin dibujar por nada?), no léxica (¿aparece el `id`?).

**Realce por conteo, ancho e higiene — modelo construido (#47, v2.21.0).** Las regiones **sin etapas** (`data-count="0"` **o atributo ausente**, tratados igual) se muestran al **50 %** mediante `fill-opacity`/`stroke-opacity` (canal de **pintura**), **no** `opacity` de elemento: así **multiplica** con la opacidad por-tier del autor (#54) y con el atenuado de vecinos del motor (`.ent-dimmed`), y el **acento de hover** de esas piezas baja también a la mitad. Las regiones **con etapas** (`data-count>0`) se **«levantan»** al hover (`transform` + `drop-shadow`), sumándose al acento; **CSS (opción A)**. Quedó documentada y pre-aprobada la **opción B** (reordenar el `<path>` al frente en el motor) por si el solape molestara; **no se usó**. El mapa **llena la columna** (retirado `max-width:900px`; `wide`/`full` efectivo por instancia — absorbe **#62**). `render.php` **retira el prólogo `<?xml?>`** antes de emitir (marcado válido — absorbe **#63**). **Invariante (D-1): el activo `enterprise-eu.svg` no se modifica** (compatibilidad con las herramientas de diseño/GIS); todo ajuste del marcado vive en `render.php`.

**Adenda estética de #47 (v2.21.0) — etiqueta de la unidad enfocada.** Etiqueta en la esquina **superior derecha**, espejo del botón «volver» (visibilidad `.is-drilled` —país y región; oculta en Europa— y presets `back-light`/`back-dark`), con el **nombre español** de la unidad enfocada. **Tocó el motor** (`region-map-frontend.js`, que el cuerpo de #47 dejaba intacto; admisible: la puerta a tocar el motor ya estaba abierta con la opción B): `setRegionLabel(parseName().primary)` en `applyLevel0`/`focusCountry`/`focusRegion`; la provincia no se enfoca, así que su nombre no aparece.

**Adenda estética de #47 (v2.21.0) — monograma de marca.** Firma BE decorativa como **marca de agua** en la esquina **inferior derecha**, visible en todos los niveles. `render.php` la emite como `<img class="ent-region-map__brand">` desde `assets/images/favicon-monograma.svg` (cache-busting `filemtime`, patrón del logo de cabecera §13.15; guarda `file_exists`); **decorativa** (`aria-hidden`, `pointer-events:none`). El CSS `.ent-region-map img.ent-region-map__brand` (0,2,1) gana a `.entry-content img` y anula su `width:100%`/`margin`.

**Traza de cierre** (el detalle fase-a-fase vive en la tabla «Resueltas» de `TODO.md`): #42 Fase I.1 — v2.13.0 · #43 Fase I.2 — v2.14.0 · #44 Fase I.3 — v2.15.0 · #51 replanteamiento de la Fase I (maestro georreferenciado único) — v2.16.0 · #54 colores configurables (nivel 2, CSS + custom properties) — v2.17.0 · #64 reaparición del globo tras el zoom (reconciliación en el `onDone` de `animateVB`; etiqueta determinista en enfoque a pieza única) — v2.18.1 · #45 capa de datos de conteo/filtrado por región (Bloque II: taxonomía `regiones` + «Sincronizar» #55, par al almacén #58, bloque «Mapa de ruta con metadatos» + ingesta/puerta #56, inyección de `data-count` #57; consumo aguas abajo A+B diseñado, ejecución en #46) — v2.19.0. · #46 página de destino por región (plantilla dedicada `template-region-destino.php` + dos carruseles etapas/Tipo D + hero + globo-enlace + contexto `from_region`) — v2.20.0. · #47 realce por conteo + ancho (#62) + higiene del prólogo (#63) + adenda estética (etiqueta de región enfocada —tocó el motor— y monograma de marca) — v2.21.0. · #48 Fase III.3 globo completo (Fase A: «Rutas: N» + tokenización del globo; acciones 3/5 → #66) — v2.22.0. **Con #48 se cierra el plan del mapa #41.** · #66 (spin-off de #48) — separación y rediseño de la navegación ratón/táctil: 66.1 separar entradas (Pointer Events, sin bump) · 66.2 nav ratón del último tier — v2.22.1 · 66.3 rediseño táctil de dos toques — v2.22.2. **Cierra #66.**

Brief de planificación (archivado): `claude/requirements/closed/requirements-41-mapa-regiones-interactivo.md`.

### 13.20 Contexto de navegación del carrusel de viajes de la página de destino por región (#65)

**Contexto.** La página de destino por región (§13.19, #46) muestra dos carruseles: **etapas** (Tipo B/C) y **viajes** (Tipo D). Las cards de etapa estampan el contexto `from_region` desde #46, pero las de viaje hacían un **salto plano** (`get_permalink()` sin parámetros), decisión §3.5 de `requirements-46`. Consecuencia (verificada en código): un viaje abierto desde el carrusel 2 llegaba **sin contexto** → su «Volver» caía a `wp_get_referer()` (rama `else` de `single.php`); al bajar a una de sus etapas y volver, el *referer* del viaje pasaba a ser esa etapa → **bucle «Volver» viaje↔etapa**, sin salida a la región. La card de etapa no sufría el bucle porque su `from_region` es contexto duradero (se arrastra como ancestro a las etapas).

**Decisión.** El carrusel 2 **estampa `from_region` + `region_code` (+ `region_src`)** en la card de viaje, **igual que la card de etapa** — reabre y sustituye la §3.5 de #46 («salto plano»). En `single.php`, la rama `from_region` **bifurca el prev/next por tipo de entrada** (`_post_tipo`): un **viaje** recorre los **viajes publicados** de la región (`enterprise_region_trip_ids`, filtrado a `'publish'`, mismo origen y orden que el carrusel 2, que salta los no publicados); una **etapa** sigue recorriendo las etapas del término (`enterprise_region_stage_query`, sin cambio) — así navegación y listado no divergen en ninguno (§6, «Regla para funcionalidades análogas»). Las **etiquetas** prev/next de un viaje en contexto de región son «Viaje anterior / Siguiente viaje» (como en la colección); una etapa mantiene «Ruta…». El **«Volver» del contexto de región se unifica** en **«← Volver a la región»** para etapa y viaje (sustituye el «← Volver» previo de #46). El **discriminador es el tipo de entrada** —los dos carruseles son disjuntos por tipo (etapas B/C, viajes D)—, sin parámetro nuevo en la URL.

**Consecuencias.** El viaje lleva contexto duradero: su «Volver» resuelve a la página de región y ya no depende del *referer*; sus etapas heredan `from_region` como ancestro (vía `enterprise_nav_origin_params()`, sin tocar) y el ida-y-vuelta etapa↔viaje ya no rompe el «Volver» — **bucle eliminado**. **No** se tocó `enterprise_nav_origin_params()` ni `blocks/post-stages/render.php`: ya reconocían y arrastraban `from_region`. Precedente reutilizado: el contexto `from_col` de la «Colección de viajes» (#8, §13.10), que evita exactamente este bucle. Implementado por el Desarrollador en `c90928c` (estampado en `template-region-destino.php`) + `d3a833d` (prev/next + etiquetas + «Volver» en `single.php`), validado por Juanjo en WordPress real. Requisitos: `claude/requirements/closed/requirements-65-bucle-volver-viaje-region.md` (no versionado). **bump 2.22.2 → 2.22.3.**
