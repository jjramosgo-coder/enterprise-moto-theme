...
**Tema WordPress:** Bitácora Enterprise v2.16.0  
...
**Nota (v2.16.0 — #51):** las fases **#42/#43/#44** de abajo corresponden al **modelo *swap*** de la Fase I original (activos simplemaps por país, `europe.svg` inline, un SVG por nivel). #51 **replanteó y reemplazó** esa Fase I por un **único SVG georreferenciado**; estas notas se conservan como registro histórico, pero **el modelo vigente es el de la nota de #51 al final de la sección**.
...
**[#43 · Fase I.2 — cerrada, v2.14.0].** Alta del bloque `enterprise/interactive-region-map` (etiqueta «Mapa interactivo de regiones») en `enterprise_register_map_blocks()`: `api_version` 3, `editor_script`, `render_callback`, `supports` (`html` false + `align` wide/full); **sin clave `category` en el registro PHP** y **sin `attributes`** (los atributos por-región son #47). `render.php` emite `europe.svg` **inline** en el DOM dentro de `<div class="ent-region-map" data-map-level="1">`; aviso solo-editor si falta el activo; el SVG (activo GPL propio de confianza) se emite tal cual, **sin `wp_kses`** (eliminaría elementos/atributos SVG) y sin `<script>` inline — inline porque la interactividad de #44 necesita los nodos `<path>`. Editor: `registerBlockType` con **placeholder estático**, `save: null` (bloque dinámico, render en PHP); el JS lleva `category:'enterprise-moto'` como sus hermanos, pero al **no** existir categoría de bloques registrada resuelve a «Sin categoría» hasta #39 (el registro PHP no lleva `category`). CSS propio (`assets/css/interactive-region-map.css`): dimensionado responsive mínimo (`svg` `width` 100% / `height` auto), **encolado condicional propio** `enterprise_region_map_assets()` (`is_singular||is_page` + `has_block`, `filemtime`), **separado** de `enterprise_map_frontend_assets()`: **no carga OpenLayers** (§13.19, no reutiliza el motor). Implementado por el Desarrollador en 2 commits (`3b0a81d` esqueleto + render; `a6db4b4` contenedor responsive + encolado condicional) y validado por Juanjo en WordPress real (mapa de Europa dibujado e inerte; cabe en móvil; el CSS solo carga donde está el bloque; OpenLayers no se carga). Coordinación con **#39**: el bloque se registró **sin** categoría y se agrupará al ejecutar #39 (referencia cruzada #43↔#39 en `TODO.md`). Requisitos: `claude/requirements/closed/requirements-43-esqueleto-bloque-mapa-regiones.md` (no versionado).

**[#44 · Fase I.3 — cerrada, v2.15.0].** Motor de navegación propio en vanilla JS (`assets/js/region-map-frontend.js`, sin build) sobre el SVG inline de #43; **no reutiliza OpenLayers**. Implementado y validado por Juanjo en WordPress real en 4 commits (`c33241b` carga de niveles; `9da7a93` hover; `3c95a02` drill-down + zoom; `33fe058` icono volver). Construido: (1) **carga de niveles** por patrón `data-*`+`fetch` — nuevo `data-maps-base` en `render.php` (URI de `assets/maps/`), encolado del JS en `enterprise_region_map_assets()` (condicional `has_block`+`filemtime`, junto al CSS), `fetch` de `region-codes.json` para la estructura y mapa país→fichero como constante del JS; (2) **navegabilidad data-driven** desde `region-codes.json.countries` (ES/IT/FR/PT/AD navegables; los otros 39 países inertes); (3) **hover** con resalte + globo de solo `name` (idioma según el activo: inglés en nivel-1, local en región/provincia); (4) **drill** país→región→provincia con carga progresiva y caché por fichero (**swap** de SVG por nivel); «zoom» por transición de `viewBox` con `getBBox`+`requestAnimationFrame`, apoyado en el **lienzo compartido nivel-2/3 de cada país** (verificado: bbox `es-regiones`≡`es-provincias`); el salto nivel-1→país es swap completo (proyección `europe` Robinson ≠ país Mercator); (5) **colapso PT/AD** a su único sub-nivel; (6) **volver** = icono sin texto arriba-izquierda que sube un nivel. **Reducción de alcance deliberada (operador):** la **accesibilidad de teclado + lector de pantalla** que preveían el TODO de #44 y esta sección **se retiró** («se navega con el ratón y punto») → **#50**. **Evolución en evaluación:** el modelo hace *swap* por país (al entrar en un país desaparecen los vecinos); un zoom continuo que conserve el contexto entre países exigiría geometría georreferenciada única — medido: no se logra fusionando los activos actuales (proyecciones distintas; aspecto de la misma península 1,31 en `europe` vs 1,24 en `es-regiones`) → **#51** (en evaluación). Requisitos: `claude/requirements/closed/requirements-44-navegacion-mapa-motor-vanilla.md` (no versionado).

**[#51 · replanteamiento de la Fase I — cerrada, v2.16.0].** Sustituye el modelo *swap* (#42/#43/#44) por un **único SVG maestro georreferenciado**. Navegación validada: un solo fichero, zoom por `viewBox`, vecinos atenuados (no desaparecen), volver = alejar un nivel, móvil por toque. Ejecutado en tres sub-fases + dos extensiones, todas validadas por Juanjo en WordPress real:

- **Activos (sub-fase 1):** maestro `assets/maps/enterprise-eu.svg` (origen **Natural Earth, dominio público**; `viewBox 0 0 800 502`; capas `tier0`/`tier1`/`tier2` = 5/76/276; cada `<path>` con `id`(=code ISO 3166-2 sin guion), `data-admin`, `data-parent`, `data-name` bilingüe `Nativo [Español]`) y modelo `assets/maps/map-regions-global.json`, ambos de **BE Map Studio**. Retirados los 9 SVG de simplemaps y `region-codes.json`; licencia migrada de simplemaps a Natural Earth. Commits `9f9ad6b`/`f40c8e6`/`962d9c2`/`f862bfb`.
- **Render estático (sub-fase 2):** `render.php` emite el maestro **inline** en `<div class="ent-region-map">` (sin `data-*`); CSS oculta `tier1`/`tier2`; motor viejo desencolado. Commits `3fbc5a8`/`f53c1de`/`17eb6b5`.
- **Motor (sub-fase 3):** reescritura de `region-map-frontend.js` **DOM-only** (Decisión H: sin `fetch`; jerarquía/nombres de los `data-*`; hijos = `path[data-parent=id]`). Zoom = animación de `viewBox` al *bbox* del nodo (`getBBox`+`rAF`); visibilidad por clases `ent-active`/`ent-dimmed`/`ent-hidden`; globo **bilingüe** (Decisión G); «volver» = alejar un nivel; ratón + toque. Commits `0165db4`/`0781a7d`/`9ea970f`/`9294685` (+ `4a551d9` fondo estable en zoom).
- **Vecinos (ext. §7):** atenuado = **visible + clicable** en todos los niveles (clic = saltar por `data-admin`); a nivel región, todas las regiones salvo la enfocada quedan atenuadas, **incluidas las de otros países** (salto región↔región transfronterizo). Commits `ac5d206`/`7253624`.
- **Regiones terminales (ext. §8):** las 27 unidades admin-1 sin sub-nivel (18 distritos PT, 7 parroquias AD, Ceuta, Melilla) pasan de no-op a **foco-sobre-sí-mismas** (enfocabilidad por nivel admin; provincia = hoja/#46). Commit `3f8e284`.

**Decisiones:** **E** islas — se mantiene Canarias, el encuadre del país se cura en el motor; **F** países-contexto — solo los 5 (reversible); **G** idioma — globo bilingüe; **H** motor DOM-only. **Acoplamiento mapa↔motor:** el **único** punto con países concretos es `var OUTLIERS = { ES: ['ESCN'] }`; el resto es genérico y dirigido por el dato, así que **un maestro más amplio (p. ej. 44 países) se sustituye sin tocar código** (salvo añadir sus outliers). **#49** ya estaba **cancelado** (el modelo del mapa trae los nombres en español); la Decisión G (globo bilingüe) es coherente. **#50** (teclado/lector) sigue **abierto** (motor solo ratón/toque). Requisitos: `claude/requirements/closed/requirements-51-*` (no versionados).
